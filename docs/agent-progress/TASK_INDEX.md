...
| TASK-022 | Rà soát action Java **thiếu** `requireRole` so với JS (vd `create_stock_count`, `approve_stock_count`) | PENDING | — | 18/09/2026 | Phải đối chiếu từng action — Java có thể đã kiểm bằng `requireActionModule`; **không thêm máy móc** |
| TASK-023 | Bổ sung kiểm quyền dự án còn thiếu trong `cancel_request` (JS có `canAccessProject`, Java chưa) | PENDING | — | 18/09/2026 | Phải thêm mức quyền vào port `ProjectScopeStore` trước |
...
| TASK-B03 | **Không đóng gói lại được JAR Java** — `mvn package` bị sandbox chặn ghi `C:\Users\PC\.m2`; `mvn -o` thiếu artifact | BLOCKED | — | 17/09/2026 | Chặn mọi thay đổi backend có hiệu lực thật (TASK-019, TASK-021) |
