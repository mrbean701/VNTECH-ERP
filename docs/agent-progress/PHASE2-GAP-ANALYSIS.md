...
| 25 | 12 test case bắt buộc | — | Đã đo bộ test hiện có (xem `TASK-103.md` §2.5/§2.6) + **chạy cổng xanh**: `npm test` **exit 0** (eslint 0 error · tsc 0 · regression **69 pass/0 fail** · workflow PASS), bộ test chạy **SQLite `:memory:`** không đụng MySQL thật | bắt buộc | Test xanh là **baseline**, **chưa đủ phủ 12 case §25**: **Case 9 bất khả thi** (thiếu tính năng versioning), 5 case thiếu assert, 6 case phủ sót | Sửa engine mà không có lưới an toàn | **P0** |
