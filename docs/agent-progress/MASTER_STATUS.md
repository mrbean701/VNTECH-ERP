...
| CURRENT TASK | **Vòng rà cuối "bản đồ GHI" ĐÃ XONG** — 3 cột còn lại đã phân loại ở known issue #44: `project_archives.purge_audit_id` **CONFIRMED (thiếu audit + purge_audit_id)** · `boq_price_import_items.changed` **CONFIRMED (thiếu cột `changed`)** · `boq_versions.approved_at` **LIKELY dương tính giả** (JS bind `null`). **Việc kế tiếp:** TASK-046 (sửa 2 cột CONFIRMED) → `save_material` phần 2 → hạng mục `audit(...)` toàn hệ thống |
| LAST COMPLETED | **TASK-045 (#63)** — `save_material`: Java **xoá hệ M&E** (`system='KHAC'`) của mọi mã vật tư mỗi lần admin lưu (UI không gửi `system`, Java đọc `payload.system`) + **không ghi** `material_code_history` + bỏ `minStock` + không kiểm nhóm/không chặn đổi mã thiếu lý do. Probe **6/14 → 14/14** (có đối chứng dương). Trước đó: **TASK-043+044 (#62)** · TASK-042 (#61) · TASK-041 (#57–#59) |
...
44. **TASK-046 — vòng rà cuối 3 cột (kết quả phân loại, CHƯA sửa):**
   * **`project_archives.purge_audit_id` — CONFIRMED (lỗi port thật):** JS `system-route.mjs:2451-2452` ghi **1 dòng `audit_logs`** (`PURGE_AFTER_OFFLINE_ARCHIVE`) rồi `UPDATE project_archives SET purge_audit_id=<auditId>`; Java `ProjectAdminStoreAdapter:242` **dừng ở** `UPDATE project_archives SET status='purged',purged_at=?` ⇒ **thiếu cả hai câu** ⇒ purge dự án **không có vết audit liên kết**. (Java **có** cổng audit `AuditLogPort.log(...)`, 9 nơi gọi ⇒ sửa được, không phải "chưa có cơ chế".)
   * **`boq_price_import_items.changed` — CONFIRMED (lỗi port thật):** JS `:2827` tính `isChanged` cho từng dòng nhập giá và ghi cột `changed`; Java `BoqStoreAdapter:706` chỉ ghi `(id,batch_id,boq_item_id,old_unit_price,new_unit_price,created_at)` ⇒ **mất cờ "dòng có đổi giá"** khi xem lịch sử nhập giá.
   * **`boq_versions.approved_at` — LIKELY DƯƠNG TÍNH GIẢ:** JS `:2805` **bind thẳng `null`** cho cột này trong `INSERT INTO boq_versions (…effective_at,approved_at,…)` ⇒ công cụ bản đồ GHI đếm một giá trị NULL là "có ghi". **Cần xác nhận** không còn chỗ nào ghi giá trị thật trước khi kết luận.
45. **Cổng điều tra nay còn 7 bảng lệch BẢN ĐỒ GHI** (sau khi vá `custom_field_values` ở #62 và `material_code_history` ở #63): `boq_price_import_items` · `boq_versions` (nghi dương tính giả) · `email_outbox` (đã đăng ký: Java không có cơ chế gửi) · `project_archives` · `users.avatar_url` (dương tính giả đã biết: Java ghi qua JPA) · `vntech_license_*` (nhóm 6, chờ quyết định #11).
