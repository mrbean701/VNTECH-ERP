...
| CURRENT TASK | **TASK-040** — sửa **26 câu lệnh SQL ghi cột không tồn tại** (7 adapter) theo nhóm, mỗi nhóm kiểm chứng lúc chạy |
| LAST COMPLETED | **TASK-039 (#45)** — vá **3 chỗ lệch** làm 2 action admin lỗi **HTTP 500** + 1 cấu hình ghi được mà không đọc ra; probe lúc chạy **10/10**. Trước đó: **TASK-038 (#44)** · **TASK-037 (#43)** |
| NEXT TASK | TASK-040 (sửa từng nhóm) → TASK-034 (gỡ chặn dựng bundle) → chứng minh render §8.1 → **§8.2** → **§8.3** |
| BLOCKED ITEMS | **TASK-034** (`npm run build` không dựng lại được UI: dấu vân tay nguồn lệch + bảng identity có **trigger chặn UPDATE** ⇒ phải viết migration) · **TASK-040** (26 câu lệnh — có kế hoạch, không cần quyết định) · **TASK-035 mục 7** · **TASK-036 mục 7** · **TASK-037 mục 5** · **TASK-031** · **TASK-032** · **TASK-029** · **TASK-024** · **dữ liệu `user_module_permissions`** |
