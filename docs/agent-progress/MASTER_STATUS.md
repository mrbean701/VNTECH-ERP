...
| **4 khoá dữ liệu còn lại — CẦN RÀ TỪNG KHOÁ** | **`centralReturns` — ĐÃ XÁC NHẬN THIẾU (17/09):** quét toàn bộ tầng persistence: mọi câu liên quan `central_returns` chỉ là **đếm cho close-check** (`ProjectAdminStoreAdapter:90` → `out.put("open_central_return", COUNT…)`), **tra 1 phiếu theo id** (`WarehouseStockStoreAdapter:501`), **đường xoá/purge** và **archive/export** — **KHÔNG có truy vấn DANH SÁCH nào được đưa vào bootstrap** ⇒ màn **"Trả hàng về Kho Tổng"** sẽ **rỗng danh sách sau cutover** dù action ghi đã port. **`centralInventory` · `companyAvailability` · `constructionDailyLogItems` — CHƯA xác nhận**, cần rà tiếp theo đúng quy trình dưới |
