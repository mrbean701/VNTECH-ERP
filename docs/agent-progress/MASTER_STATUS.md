...
| CURRENT TASK | **Rà tiếp các bảng lệch BẢN ĐỒ GHI** — kế tiếp: `material_code_history`, `project_archives.purge_audit_id`, `boq_versions.approved_at`, `boq_price_import_items.changed`; **việc MỚI đã đăng ký:** (a) Java **thiếu `audit(...)`** ở `RequestManagementUseCase` (đo được: `audit_logs.entity_type='material_request'` = **0 dòng**) · (b) Java **yếu hơn JS 3 phép kiểm Owner** của bước duyệt · (c) `delete_request` (**cả JS lẫn Java**) để rác `procurement_allocations` + `custom_field_values` |
| LAST COMPLETED | **TASK-043 + TASK-044 (#62)** — trường động của DÒNG phiếu: Java **chưa bao giờ GHI** `custom_field_values` **và ĐỌC sai chỗ** (tra bằng id PHIẾU thay vì id DÒNG ⇒ `customFields` luôn rỗng); sửa cả hai chiều + port `routeTag`; **TASK-044**: bootstrap **bỏ sót 11 trường** của dòng phiếu (UI luôn hiện 0 ở `remainingQty`/`linkedPoCount`/…); probe **26/26**. Trước đó: **TASK-042 (#61)** · TASK-041 phần 1–3 (#57–#59) |
...
* **Infrastructure**: MySQL **3306** · Java API **18081** (PID **2016**, background job `pwsh-78`) · Node SSR **8787** · cutover proxy **9000** (người dùng mở `:9000`)
* **JAR**: đã đóng gói lại **thành công** — `vntech-erp-web-0.1.0-SNAPSHOT.jar` **90.893.269 bytes** (17/09 16:53); **mọi sửa đổi backend ĐÃ có hiệu lực lúc chạy**
...
* [ ] **Rà tiếp:** `material_code_history` (`save_material` — Java **không nhắc bảng này**) · `project_archives.purge_audit_id` · `boq_versions.approved_at` · `boq_price_import_items.changed`
