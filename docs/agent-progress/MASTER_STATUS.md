...
| CURRENT TASK | **Rà tiếp các bảng lệch BẢN ĐỒ GHI** — kế tiếp: `project_archives.purge_audit_id`, `boq_versions.approved_at`, `boq_price_import_items.changed`; **đã đăng ký:** (a) Java **thiếu `audit(...)`** ở luồng Phiếu đề nghị **và** ở `save_material` (đo được: `audit_logs.entity_type='material_request'` = **0 dòng**) · (b) Java **yếu hơn JS 3 phép kiểm Owner** của bước duyệt · (c) `delete_request` (**cả JS lẫn Java**) để rác `procurement_allocations` + `custom_field_values` · (d) **`save_material` phần 2** (rebuild `aliasText` + luật trùng tên + audit + văn bản thông điệp) · (e) `merge_material_master` giữ mã nguồn thay vì xoá |
