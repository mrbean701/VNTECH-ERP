...
| CURRENT TASK | **TASK-050 — port 4 khoá bootstrap thiếu** (`centralInventory` · `centralReturns` · `constructionDailyLogItems` · `companyAvailability`): đã **xác nhận hai phía** là lỗi port đường ĐỌC của Java; đặc tả 6 bước + cảnh báo **phải port kèm bộ lọc quyền** (`:622`, `:732`, `:733`) đã ghi ở `docs/agent-progress/TASK-050.md`; nghiệm thu = `probe-bootstrap-keys.mjs` phải **RỖNG**. **Xếp trước TASK-048** vì đây là dữ liệu người dùng nhìn hằng ngày. Kế tiếp: **TASK-048** (6 audit luồng Phiếu đề nghị — phép kiểm ĐỎ sẵn trong repo, hiện 19/20) · **TASK-049** (3 phép kiểm Owner + `merge_material_master`) · mở rộng audit theo cổng đo |e_material` nay đã có audit ở #68). Kế tiếp: **TASK-049** 3 phép kiểm Owner của bước duyệt · `merge_material_master` (JS xoá mã nguồn, Java chỉ `active=0`) |
...
| Việc kế tiếp | **port 4 truy vấn ĐỌC** nói trên vào bootstrap Java (kèm bộ lọc quyền), rồi chạy lại `tools/probe-bootstrap-keys.mjs` để khẳng định danh sách "UI đọc mà Java không trả" **rỗng** |
