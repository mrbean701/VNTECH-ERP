...
| CURRENT TASK | **TASK-047 — `save_material` phần 2**: rebuild alias theo `aliasText` (JS xoá hết rồi tạo lại, bỏ tên gốc/trùng) · **luật trùng tên gốc + alias** (3 thông điệp JS) · bỏ alias tự-tạo-bằng-tên-gốc · `audit(...)` cho `CREATE`/`UPDATE` material · văn bản thông điệp theo JS. Kế tiếp: **TASK-048** hạng mục `audit(...)` toàn hệ thống · **TASK-049** 3 phép kiểm Owner của bước duyệt |
| LAST COMPLETED | **TASK-046 (#67)** — vòng rà cuối bản đồ GHI: `project_archives.purge_audit_id` (Java **thiếu cả câu audit lẫn câu liên kết id**) + `boq_price_import_items.changed` (**thiếu cột `changed`**); probe còn bắt **lỗi P0**: `delete_project` **luôn HTTP 500** vì MySQL từ chối DATETIME `'0000'` (JS/SQLite so chuỗi nên vô hại) ⇒ **không admin nào purge được dự án**. Probe **17/17**. Trước đó: **TASK-045 (#63)** · **TASK-043+044 (#62)** · **TASK-042 (#61)** |
