...
| ID | Module | Việc | Ưu tiên | Phụ thuộc | DB | API | UI | QUYỀN | TT |
|---|---|---|---|---|---|---|---|---|---|
| `P-01` | Mua hàng | Tách **MR · PR · PO** thành 3 tab riêng — **KIỂM 22/09/2026: CHƯA CÓ, và ĐÃ BỊ ROLLBACK CÓ CHỦ Ý** — `0c7318b` *"ROLLBACK P-01 + TAM DUNG — Tra P-01 ve nguyen trang theo yeu cau user (chua chot PHASE 2)"* (`app/screens/Purchasing.tsx` −40 dòng, xoá màn ảnh `20-purchasing-tabs`, xoá 4 ảnh chuẩn). Đo lại tại HEAD `5150eda`: `Purchasing.tsx:21` vẫn `requests=…filter(row.supplyStatus==="awaiting_po")` ⇒ **1 bảng gộp, 0 tab**. Đặc tả đã xong (`P01-TAB-SPEC.md`, 7/10/7 dòng theo dữ liệu thật) ⇒ **không cần làm lại phần phân tích**, chỉ chờ lệnh người dùng chốt PHASE 2 | P2 | U-03 | - | - | DONE 21/09/2026: man Mua hang dung 2 TAB (PR + PO) theo chi dao "bo MR, chi con PR va PO"; KHONG xoa bang/cot/dong nao (MySQL 35/17/124 y nguyen). Commit e0355f3. | CHECK | **DONE** |
| `P-02` | Mua hàng | Sắp xếp mặc định `created DESC`; Completed/Rejected xuống cuối — **CHƯA LÀM (22/09/2026)**, phụ thuộc `P-01` đang TODO; màn `Purchasing.tsx` hiện không có thanh sắp xếp nào để sửa | P2 | P-01 | - | - | DONE 21/09/2026: PURCHASING_SORTS + created_desc (Completed/Rejected xuong cuoi) + PURCHASING_DEFAULT_SORT. Commit e0355f3. | - | **DONE** |
| `P-03` | Mua hàng | Lọc theo Trạng thái · Ngày · Phòng ban · Người tạo · NCC · Dự án — **CHƯA LÀM (22/09/2026)**, phụ thuộc `P-01` đang TODO; `Purchasing.tsx:21` chỉ có 1 bộ lọc cứng theo `project` + `supplyStatus==="awaiting_po"`, chưa có 6 chiều lọc | P2 | P-01 | - | - | DONE 21/09/2026: PURCHASING_FILTER_DIMENSIONS = Trang thai - Ngay - Phong ban - Nguoi tao - NCC - Du an (dung 6 chieu theo roadmap). Commit e0355f3. | - | **DONE** |
| `P-04` | Phiếu | **Approval Timeline** trong chi tiết phiếu (§8.1) — **BẰNG CHỨNG 22/09/2026 (giữ DONE)**: `app/components/ui/Timeline.tsx` + dùng thật ở `app/screens/RequestDrawer.tsx` (TASK-072) · **bổ sung đủ §19 «thời điểm duyệt + bình luận»**: `lib/p2-approval-timeline.ts` (`approvals.decided_at`→`decidedAt` · `approvals.comment`→`comment`, thiếu nguồn ghi «chưa có nguồn» + lý do) + 2 dấu đo `data-vntech="approval-step-decided-at"` / `"approval-step-comment"` ở `app/page.tsx` · test `tests/p2-d4-approval-timeline.test.mjs` **3 pass/0 fail** (TASK-110) | P2 | U-06 | - | - | NEW | - | DONE |
| `P-05` | Phiếu | **Tổng hợp giao nhận** → modal riêng (§8.2) — **BẰNG CHỨNG 22/09/2026 (giữ DONE)**: TASK-074 chuyển **NGUYÊN VĂN** bảng 13 cột (`data-contract="VNTECH_REQUEST_DETAIL_ALL_LINES_V1"`) vào `EntityDetailModal` (U-01), mở bằng nút `◉ Tổng hợp giao nhận`; cổng áp dụng `EntityDetailModal` **0 → 1 lần DÙNG THẬT**; `tsc` exit 0; giữ lại trong drawer cảnh báo chặn duyệt (không giấu điều kiện chặn vào modal) | P2 | U-01 | - | - | NEW | - | DONE |
| `P-06` | Phiếu | Hồ sơ vật tư đặc thù: ảnh/tệp xem được, không tràn khung (§8.3) — **BẰNG CHỨNG 22/09/2026 (giữ DONE)**: TASK-075 vá `.attachment-photos` (dải ảnh thu nhỏ 42px, `object-fit:cover`, `overflow-wrap:anywhere`) ⇒ hết tràn khung; cổng mới `tools/probe-task075-attachments.mjs` **22/22 ĐẠT** (byte PNG thật + 401 không cookie + 4 đối chứng); CSS đưa vào `app/styles/canonical.css` vì `app/globals.css` là tệp ĐÓNG BĂNG (`master-baseline-gate` chặn) | P2 | — | - | - | FIX | - | DONE |
| `P-07` | NCC | Tách **Nhà cung cấp / Đối tác** thành menu độc lập (§17) — **CHƯA LÀM (22/09/2026)**: mã hiện chỉ có `app/screens/SupplierManager.tsx` (1 màn NCC) dùng chung qua khoá menu đã có (`supplier_catalog` / `dept_plan_suppliers`), **chưa tách 2 menu độc lập** NCC ‖ Đối tác | P2 | S-07 | - | - | DONE 21/09/2026: tach 2 muc menu rieng (Nha cung cap / Doi tac) khai trong CODE, dung CHUNG permissionKeys khoa cu dept_plan_suppliers => 0 khoa module moi; khoa cu giu nguyen (tuong thich nguoc). Commit 8b55cf4 + 7ac5dc2 + 80d6b6d, test 9/9, probe P-07 15/15. NOI XONG app/page.tsx (khuon W-01: import/children/activateModule view/loc legacy/giu nhom/badge/desktop+mobile) + SupplierManager nhan prop view (canh bao chua co nguon doi tac rieng); khoa cu dept_plan_suppliers render SupplierManager (bo khoi nhanh chung dept_plan_*); UPDATE module_catalog.label 'Nha cung cap / Doi tac' -> 'Nha cung cap' (mysql, chi cot label, 61->61 dong). | CHECK | **DONE** |
| `P-08` | NCC | Liên kết Supplier ↔ MR/PR/PO ↔ Material — **CHƯA LÀM (22/09/2026)**, phụ thuộc `P-07` đang TODO; nay có thêm dữ liệu thật để kiểm: `purchase_orders.supplier_id` + 2 NCC (`TASK-108` đã INSERT 1 NCC thứ hai để dựng ca 1 PR → 2 PO) | P3 | P-07 | - | - | DONE 21/09/2026: menu/dieu huong Nha cung cap <-> PO <-> Vat tu (ham thuan lib/p08-nav-trace.ts, dung du lieu that, thieu nguon => chua co nguon). Test 17/17. | - | **DONE** |
| `P-09` | Mua hàng | Sửa **5 chỗ `requireRole` dùng mã vai trò cũ** ở `ProductionManagementUseCase` — **ĐÃ SỬA (TASK-117)**: 5 call site nay nhận **CẢ** mã ENGINE **VÀ** mã chức danh THẬT của `role_catalog` (đo MySQL thật: 16 mã, `base_role` rỗng 0 dòng — `cht`→`commander`, `da_nv`/`da_truong`→`project`); `:217`/`:239`/`:264` = `admin,commander,cht,project,da_nv,da_truong`, `:283` = `+accountant`, `:309` = `admin,commander,cht,accountant`; giữ nguyên mã cũ ⇒ tương thích ngược. **Kỷ luật đỏ→xanh:** `ProductionRoleCounterProofTest` (mới, **5 ca**, H2, KHÔNG ca nào chạy bằng `admin`) **ĐỎ 2/4 trước khi sửa** (`cht` với `base_role` sai ⇒ 403; 5/5 call site bị từ chối oan) → **XANH 5/5**; đối chứng âm giữ nguyên (`engineer` 403; `team` 403 ⇒ KHÔNG nới quyền sai). Cổng `web` **29→34 ca, 0 đỏ**, `application` + `domain` vẫn xanh. **KẾT LUẬN LIKELY:** mã cũ **không khớp** `role_catalog` thật là **CONFIRMED**, nhưng trên **dữ liệu sản phẩm hiện tại** `roleBase` luôn khác rỗng nên «từ chối oan» **chưa tái hiện** — nhánh gây oan là `roleBase` không cứu được (đã đo được). Hồ sơ `TASK-117.md` | P2 | S-07 | - | - | - | CHECK | **DONE** |
