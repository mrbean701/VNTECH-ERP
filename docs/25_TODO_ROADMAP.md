...
| `P-09` | Mua hàng | Sửa **5 chỗ `requireRole` dùng mã vai trò cũ** ở `ProductionManagementUseCase` — **CHƯA LÀM (22/09/2026)**: kế hoạch đã chứng minh (`P09-FIX-PLAN.md`, commit `76c6a5e` *"chua ap dung"*); ĐO LẠI tại HEAD `5150eda`: 5 lời gọi **vẫn nguyên mã cũ** `ProductionManagementUseCase.java:217/239/264` = `List.of("admin","commander","project")`, `:283`/`:309` = `List.of("admin","commander","accountant"[,"project"])`. ⚠️ **ĐÍNH CHÍNH TIỀN ĐỀ:** `RbacService.java:80` nay kiểm **CẢ HAI** (`roles.contains(user.role())` **và** `user.roleBase()`) ⇒ chưa có bằng chứng «từ chối oan» còn tái hiện; nhưng tài liệu roadmap còn dòng cũ nên chưa đủ căn cứ đóng — thiếu 1 ca chứng minh user `role=cht`/`da_truong` (có `roleBase`) ĐƯỢC phép và `engineer` bị chặn | P2 | S-07 | - | - | - | CHECK | TODO |
