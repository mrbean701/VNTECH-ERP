...
function LegalDocsScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  // MT2 §10.4 (②A) — nguồn để CHỌN/TRA CỨU công văn liên kết. ⛔ KHÔNG tạo bảng mới, ⛔ KHÔNG
  // nhân bản dữ liệu công văn: dùng thẳng payload `officialCorrespondence` (Java: BootstrapDataAdapter).
  const correspondences=data.officialCorrespondence||[];
  /** Nhãn hiển thị của một công văn: «số công văn · trích yếu». */
  const corrLabel=(c:Row)=>`${c.docNo||""}${(c.summary||c.docType)?` · ${c.summary||c.docType}`:""}`;
  ...
  <section className="card"><CardHead title="Văn bản pháp lý" note="Hợp đồng, phụ lục, quyết định, giấy phép; theo dõi hiệu lực và file đính kèm."/>{permission.canCreate&&<form className="payment-entry-inline legal-doc-form" onSubmit={saveDoc}><input name="docNo" placeholder="Số VB *" required/><select name="docType" defaultValue=""><option value="">Loại *</option><option>Hợp đồng</option><option>Phụ lục</option><option>Quyết định</option><option>Giấy phép</option><option>Công văn quy phạm</option><option>Khác</option></select><input name="title" placeholder="Tiêu đề *" required/><input name="issueDate" type="date"/><input name="issuer" placeholder="Cơ quan ban hành"/><input name="effectiveDate" type="date"/><input name="expiryDate" type="date"/><input name="scope" placeholder="Phạm vi áp dụng"/><select name="correspondenceId" defaultValue=""><option value="">Công văn liên kết (không bắt buộc)</option>{correspondences.map((c:Row)=><option key={c.id} value={c.id}>{corrLabel(c)}</option>)}</select><button className="primary">＋ Lưu VB</button></form>}
  <div className="table-wrap"><table><thead><tr><th>Số VB</th><th>Loại</th><th>Tiêu đề</th><th>Ban hành</th><th>Cơ quan</th><th>Hiệu lực</th><th>Hết hạn</th><th>Phạm vi</th><th>Công văn liên kết</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map((r)=><tr key={r.id}><td>{r.docNo}</td><td>{r.docType}</td><td><strong>{r.title}</strong></td><td>{r.issueDate||"—"}</td><td>{r.issuer||"—"}</td><td>{r.effectiveDate||"—"}</td><td>{r.expiryDate||"—"}</td><td>{r.scope||"—"}</td><td>{r.correspondenceId?(correspondences.find((c:Row)=>c.id===r.correspondenceId)?.docNo||r.correspondenceId):"—"}</td><td><StatusBadge value={String(r.status)==="active"?"Hiệu lực":"Hết hiệu lực"}/></td><td>{permission.canEdit&&<><button className="export-mini" onClick={()=>action("set_legal_document_status",{docId:r.id,status:String(r.status)==="active"?"inactive":"active"})}>{String(r.status)==="active"?"Hết HL":"Mở lại"}</button><button className="export-mini danger" onClick={()=>window.confirm("Xóa văn bản?")&&action("delete_legal_document",{docId:r.id})}>Xóa</button></>}</td></tr>)}{!rows.length&&<tr><td colSpan={11}><Empty text="Chưa có văn bản pháp lý."/></td></tr>}</tbody></table></div></section>
  </div>;
}
export {
  LegalDocsScreen,
