    ...
    {tab === 2 && (() => {
      ...
      return <div className="stack">
        <section className="card">
          ...
          <DataTable rows={rows} rowKey={(r) => String(String(r.u.id))} columns={[{ key: "c1", header: "Nhân viên", render: (r) => <><strong>{r.u.fullName}</strong><small>{r.u.employeeCode || r.u.username || ""}</small></> }, { key: "c2", header: "Phòng ban", render: (r) => <>{r.u.organizationName || r.u.department || "—"}</> }, { key: "c3", header: "Chức vụ", render: (r) => <>{r.u.roleName || r.u.role || "—"}</> }, { key: "c4", header: "Tổng việc", render: (r) => <>{r.total}</> }, { key: "c5", header: "Hoàn thành", render: (r) => <><strong>{r.done}</strong></> }, { key: "c6", header: "Quá hạn", render: (r) => <>{r.late}</> }, { key: "c7", header: "Tỉ lệ", render: (r) => <><div className="task-bar"><span><i style={{ width: `${r.rate}%` }} /></span><b>{r.rate}%</b></div></> }, { key: "c8", header: "Tháng này", render: (r) => <>{r.mDone}/{r.mTotal}</> }]} emptyText="Chưa có dữ liệu KPI." />
        </section>
