...
function Pill({ value }: { value: string }) { const lower = String(value).toLowerCase(); const tone = lower.includes("đã") || lower.includes("đủ") || lower === "đạt" ? "green" : lower.includes("từ") || lower.includes("chặn") || lower.includes("trễ") || lower.includes("âm") || lower.includes("không") ? "red" : lower.includes("chờ") || lower.includes("đang") || lower.includes("thiếu") || lower.includes("partial") || lower.includes("dưới") ? "amber" : "blue"; return <span className={`pill ${tone}`}><i />{value}</span>; }
...
function Requests({ rows, projects, project, onProject, open, inventory, exportRows }: { rows: Row[]; projects: Row[]; project:string; onProject:(value:string)=>void; open: (name: string, row?: Row) => void; inventory: Row[]; exportRows: () => void }) {
  ...
  return <div className="stack module-screen requests-screen approved-module-screen">
    <div className="approved-module-head"><div><h2>PHIẾU ĐỀ NGHỊ MUA HÀNG</h2><p>Nhu cầu của dự án/BCH; không gắn Tổ đội tại bước đề nghị.</p></div><div className="screen-actions"><button className="primary" onClick={()=>open("request")}>＋ Lập phiếu đề nghị</button><button className="secondary" onClick={()=>open("request")}>⇧ Nhập Excel</button><button className="secondary" onClick={exportRows}>⇩ Xuất Excel</button><button className="secondary" disabled={!selected} onClick={()=>selected&&open("detail",selected)}>◉ Xem chi tiết</button></div></div>
...
function WarehouseReceipt({ data, project, open, canUse }: { data: AppData; project: string; open: (name: string, row?: Row) => void; canUse: boolean }) {
  ...
  return <div className="stack module-screen warehouse-receipt-screen baseline-screen">
    <div className="approved-module-head"><div><h2>NHẬP KHO</h2><p>Ghi nhận chứng từ nhập kho sau giao hàng và xác nhận BCH; không dùng chung màn hình kế hoạch giao hàng.</p></div><button className="primary" disabled={!canUse} onClick={()=>open("receipt")}>＋ TẠO PHIẾU NHẬP</button></div>
    ...
    <section className="card baseline-filter-card"><div className="filter-grid receiving-filter-grid"><label><span>Trạng thái xác nhận</span><select value={status} onChange={(event)=>setStatus(event.target.value)}><option value="ALL">Tất cả trạng thái</option><option value="pending">Chờ BCH xác nhận</option><option value="confirmed">Đã xác nhận</option><option value="rejected">Từ chối</option></select></label><label><span>Tìm kiếm</span><input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Số phiếu, PO, NCC, kho..."/></label><button className="secondary" onClick={()=>{setQuery("");setStatus("ALL");}}>Đặt lại</button></div></section>
    <section className="card"><CardHead title="Danh sách phiếu nhập kho" note={`${rows.length} hồ sơ đúng phạm vi dự án/kho`}/><div className="table-wrap"><table className="baseline-table"><thead><tr><th>Phiếu nhập</th><th>PO</th><th>Dự án</th><th>Nhà cung cấp</th><th>Kho nhận</th><th>Ngày nhận</th><th>SL chấp nhận</th><th>CO/CQ</th><th>Xác nhận BCH</th><th>Thao tác</th></tr></thead><tbody>{rows.map((row)=><tr key={row.id}><td><strong>{row.receiptNo||"—"}</strong></td><td>{row.poNo||"—"}</td><td>{row.projectName||row.projectCode||"—"}</td><td>{row.supplierName||"—"}</td><td>{row.warehouseName||"—"}</td><td>{date(row.receivedAt)}</td><td>{format.format(Number(row.acceptedQty||0))}</td><td><Pill value={row.certificateStatus==="complete"?"Đầy đủ":"Thiếu hồ sơ"}/></td><td><Pill value={row.bchConfirmationStatus==="confirmed"?"Đã xác nhận":row.bchConfirmationStatus==="rejected"?"Từ chối":"Chờ xác nhận"}/></td><td><button className="icon-mini" onClick={()=>open("receiptDetail",row)}>◉</button></td></tr>)}{!rows.length&&<tr><td colSpan={10}><Empty text="Chưa có phiếu nhập kho phù hợp bộ lọc."/></td></tr>}</tbody></table></div></section>
  </div>;
}
...
function Inventory({ data, project, open }: { data: AppData; project: string; open: (name: string, row?: Row) => void }) {
  ...
  return <div className="stack baseline-screen approved-inventory-screen">
    <div className="approved-module-head"><div><h2>TỒN KHO & ĐIỀU CHUYỂN</h2><p>Theo dõi tồn theo đúng dự án/kho được phân quyền; điều chuyển phải có xác nhận kho đích.</p></div><div className="row-actions"><button className="secondary" onClick={()=>exportInventoryXlsx(filtered)}>⇩ Xuất Excel</button><button className="secondary" disabled={!filtered.length} onClick={()=>printInventoryBarcodes(filtered)}>▥ In mã Barcode</button></div></div>
...
function ProductionReports({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  return <div className="stack module-screen production-screen"><div className="kpi-grid small"><Kpi icon="SL" label="Sản lượng thực tế báo cáo" value={money(actual)} note={`${rows.length} kỳ báo cáo`}/><Kpi icon="✓" label="Sản lượng được duyệt" value={money(approved)} note="Nguồn chuẩn cho KPI/thu hồi vốn" tone="green"/><Kpi icon="!" label="Chờ phê duyệt" value={String(rows.filter(row=>row.status!=="approved").length)} note="Không suy ra từ xuất kho" tone="amber"/></div>
    <section className="card"><CardHead title="Báo cáo sản lượng theo tháng" note="BCH nhập sản lượng thi công/nghiệm thu thực tế. Số xuất kho tuyệt đối không được dùng thay sản lượng."/>{selected&&permission.canCreate&&<form className="payment-entry-inline production-entry-form" onSubmit={save}><input name="reportPeriod" type="month" defaultValue={UI_TODAY.slice(0,7)} required/><input name="referenceNo" placeholder="Số báo cáo / biên bản"/><input name="description" placeholder="Nội dung / phạm vi nghiệm thu"/><input name="plannedValue" type="number" min="0" step="1" placeholder="Giá trị kế hoạch" required/><input name="actualValue" type="number" min="0" step="1" placeholder="Sản lượng thực tế" required/><button className="primary">GỬI KIỂM TRA</button></form>}<div className="table-wrap"><table><thead><tr><th>Dự án</th><th>Kỳ</th><th>Tham chiếu</th><th>Kế hoạch</th><th>Thực tế báo cáo</th><th>Được duyệt</th><th>Người báo cáo</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map(row=><tr key={row.id}><td><strong>{row.projectCode}</strong><small>{row.projectName}</small></td><td>{row.reportPeriod}</td><td>{row.referenceNo||"—"}</td><td>{money(row.plannedValue)}</td><td><strong>{money(row.actualValue)}</strong></td><td><strong>{money(row.approvedValue)}</strong></td><td>{row.submittedByName||"—"}</td><td><Pill value={row.status==="approved"?"Đã duyệt":"Chờ duyệt"}/></td><td>{row.status!=="approved"&&permission.canApprove&&<button className="mini-approve" onClick={()=>approve(row)}>Duyệt →</button>}</td></tr>)}{!rows.length&&<tr><td colSpan={9}><Empty text="Chưa có báo cáo sản lượng thực tế."/></td></tr>}</tbody></table></div></section></div>;
}
...
function BenefitsScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Mã</th><th>Nhân sự</th><th>Loại</th><th>Đơn vị</th><th>Từ</th><th>Đến</th><th>Mức đóng</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map((r)=><tr key={r.id}><td>{r.benefitNo}</td><td><strong>{r.fullName}</strong></td><td>{r.benefitType}</td><td>{r.provider||"—"}</td><td>{r.startDate||"—"}</td><td>{r.endDate||"—"}</td><td>{money(r.monthlyAmount)}</td><td><Pill value={String(r.status)==="active"?"Đang tham gia":"Đã dừng"}/></td><td>{permission.canEdit&&<><button className="export-mini" onClick={()=>action("set_benefit_record_status",{benefitId:r.id,status:String(r.status)==="active"?"inactive":"active"})}>{String(r.status)==="active"?"Dừng":"Mở lại"}</button><button className="export-mini danger" onClick={()=>window.confirm("Xóa bản ghi?")&&action("delete_benefit_record",{benefitId:r.id})}>Xóa</button></>}</td></tr>)}{!rows.length&&<tr><td colSpan={9}><Empty text="Chưa có bản ghi bảo hiểm & chế độ."/></td></tr>}</tbody></table></div></section>
  </div>;
}
...
function CorrespondenceScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Số CV</th><th>Hướng</th><th>Loại</th><th>Ngày</th><th>Gửi / Nhận</th><th>Trích yếu</th><th>Xử lý nội bộ</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map((r)=><tr key={r.id}><td>{r.docNo}</td><td><Pill value={String(r.direction)==="IN"?"Đến":"Đi"}/></td><td>{r.docType}</td><td>{r.issueDate||"—"}</td><td>{r.receiverName||r.senderName||"—"}</td><td>{r.summary||"—"}</td><td>{r.internalHandler||"—"}</td><td><Pill value={r.status==="received"?"Đã nhận":r.status==="processing"?"Đang xử lý":r.status==="done"?"Hoàn tất":r.status}/></td><td>{permission.canEdit&&<><button className="export-mini" onClick={()=>window.confirm("Đánh dấu hoàn tất?")&&action("set_correspondence_status",{corrId:r.id,status:"done"})}>Hoàn tất</button><button className="export-mini danger" onClick={()=>window.confirm("Xóa công văn?")&&action("delete_correspondence",{corrId:r.id})}>Xóa</button></>}</td></tr>)}{!rows.length&&<tr><td colSpan={9}><Empty text="Chưa có công văn."/></td></tr>}</tbody></table></div></section>
  </div>;
}
...
function DocumentsScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Số CT</th><th>Ngày</th><th>Loại</th><th>Dự án</th><th>Nội dung</th><th>Giá trị</th><th>Người lập</th><th></th></tr></thead><tbody>{rows.map((r)=><tr key={r.id}><td>{r.voucherNo}</td><td>{r.voucherDate}</td><td><Pill value={r.voucherType}/></td><td>{r.projectCode||"—"}</td><td>{r.description||"—"}</td><td><strong>{money(r.totalAmount)}</strong></td><td>{r.createdByName||"—"}</td><td>{permission.canEdit&&<button className="export-mini danger" onClick={()=>window.confirm("Xóa chứng từ?")&&action("delete_accounting_voucher",{voucherId:r.id})}>Xóa</button>}</td></tr>)}{!rows.length&&<tr><td colSpan={8}><Empty text="Chưa có chứng từ kế toán."/></td></tr>}</tbody></table></div></section>
  </div>;
}
...
function SiteCostScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Phiếu</th><th>Dự án</th><th>Loại</th><th>Số tiền</th><th>Người chi</th><th>Ngày</th><th>Diễn giải</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map((r)=><tr key={r.id}><td>{r.claimNo}</td><td><strong>{r.projectCode}</strong></td><td>{r.costType}</td><td><strong>{money(r.amount)}</strong></td><td>{r.paidByName||"—"}</td><td>{r.claimDate||"—"}</td><td>{r.description||"—"}</td><td><Pill value={r.status==="approved"?"Đã duyệt":r.status==="submitted"?"Chờ duyệt":"Bản nháp"}/></td><td>{r.status!=="approved"&&permission.canApprove&&<button className="mini-approve" onClick={()=>action("approve_site_expense_claim",{claimId:r.id})}>Duyệt →</button>}{permission.canEdit&&r.status!=="approved"&&<button className="export-mini danger" onClick={()=>window.confirm("Xóa chi phí?")&&action("delete_site_expense_claim",{claimId:r.id})}>Xóa</button>}</td></tr>)}{!rows.length&&<tr><td colSpan={9}><Empty text="Chưa có chi phí hiện trường."/></td></tr>}</tbody></table></div></section>
  </div>;
}
function AdvanceScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Phiếu</th><th>Người nhận</th><th>Dự án</th><th>Mục đích</th><th>Loại</th><th>Số tiền</th><th>Đã chi</th><th>Hoàn ứng</th><th>Trạng thái</th><th></th></tr></thead><tbody>{rows.map((r)=>{const balance=Math.max(0,Number(r.advancePaid||0)-Number(r.settlementValue||0));return <tr key={r.id}><td>{r.requestNo}</td><td><strong>{r.requesterName}</strong></td><td>{r.projectCode||"—"}</td><td>{r.purpose}</td><td>{r.category}</td><td><strong>{money(r.amount)}</strong></td><td>{money(r.advancePaid)}</td><td>{money(r.settlementValue)}</td><td><Pill value={r.status==="approved"?"Đã duyệt":r.status==="settled"?"Đã hoàn ứng":r.status==="submitted"?"Chờ duyệt":"Bản nháp"}/></td><td>{permission.canEdit&&String(r.status)==="draft"&&<button className="export-mini danger" onClick={()=>window.confirm("Xóa phiếu tạm ứng?")&&action("delete_advance_request",{requestId:r.id})}>Xóa</button>}{permission.canApprove&&["approved","submitted"].includes(String(r.status))&&<button className="mini-approve" onClick={()=>{const s=window.prompt("Nhập số tiền đã chi tạm ứng:",String(r.amount||0));const st=window.prompt("Nhập giá trị hoàn ứng:",String(Math.min(Number(s||0),Number(r.amount||0))));if(s!==null&&st!==null)void action("settle_advance_request",{requestId:r.id,advancePaid:Number(s||0),settlementValue:Number(st||0)});}}>Hoàn ứng</button>}</td></tr>;})}{!rows.length&&<tr><td colSpan={10}><Empty text="Chưa có phiếu tạm ứng."/></td></tr>}</tbody></table></div></section>
  </div>;
}
...
function ConstructionScreen({data,project,action,permission}:{data:AppData;project:string;action:(name:string,payload:Row)=>Promise<boolean>;permission:Row}){
  ...
  <div className="table-wrap"><table><thead><tr><th>Ngày</th><th>Số phiếu</th><th>Ca / Thời tiết</th><th>Nội dung</th><th>Nhân công</th><th>Hạng mục</th><th>Khối lượng</th><th>Trạng thái</th><th></th></tr></thead><tbody>{logs.map((l)=>{const items=itemsByLog(String(l.id));return <tr key={l.id}><td>{l.workDate}</td><td>{l.logNo}</td><td>{l.shift}<small>{l.weather||"—"}</small></td><td><strong>{l.workContent||"—"}</strong></td><td>{l.laborCount||0}</td><td>{l.itemCount||items.length}</td><td>{money(l.completedQty)}</td><td><Pill value={l.status==="approved"?"Đã duyệt":l.status==="submitted"?"Chờ duyệt":"Bản nháp"}/></td><td>{l.status!=="approved"&&permission.canApprove&&<button className="mini-approve" onClick={()=>action("approve_construction_daily_log",{logId:l.id})}>Duyệt →</button>}{permission.canEdit&&l.status!=="approved"&&<button className="export-mini danger" onClick={()=>window.confirm(`Xóa nhật ký ${l.logNo}?`)&&action("delete_construction_daily_log",{logId:l.id})}>Xóa</button>}</td></tr>;})}{!logs.length&&<tr><td colSpan={9}><Empty text="Chưa có nhật ký thi công trong phạm vi đang chọn."/></td></tr>}</tbody></table></div></section>
  </div>;
}
