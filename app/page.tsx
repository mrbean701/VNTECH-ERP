...
function RequestModal({ data, contextProject, close, submit }: { data: AppData; contextProject:string; close: () => void; submit: (name: string, payload: Row) => Promise<boolean> }) {
  ...
  return <BaseModal title="Lập đề nghị cấp vật tư" note="Có thể chọn Dự án → Hợp đồng → BOQ Version để đối chiếu khối lượng (không bắt buộc). File chỉ nhập nhu cầu; hệ thống tự lấy khối lượng HĐ, lũy kế, tồn kho và hàng đang chờ giao. Sau khi gửi, phiếu vào luồng phê duyệt ngay và không tự thu hồi." close={close}><form className="request-form" onSubmit={send}><div className="modal-body"><div className="request-company-head"><div><img src={VNTECH_BRAND.logoPath} alt="VNTECH"/><strong>ĐỀ NGHỊ CẤP VẬT TƯ</strong></div><p>Kính gửi: <b>GIÁM ĐỐC CÔNG TY, CÁC PHÒNG/BAN CÔNG TY</b></p><small>Số phiếu tự động: {`DNMH-${project?.code||"DAxx"}-${new Date().getFullYear()}-xxxx`}</small></div><div className="form-grid request-context-grid">
    <label><span>{label("projectId","Dự án")}</span>{lockedProject||data.projects.length===1?<strong className="locked-project-value">{project?.code} · {project?.name}</strong>:<select name="projectId" value={projectId} onChange={(e)=>{setProjectId(e.target.value);setContractSelection("");setBoqVersionSelection("");setLines([{materialId:"",materialCode:"",materialName:"",unit:"",quantity:1,unitPrice:0,boqItemId:"",contractLineNo:"",origin:"",approvedSupplier:"",installationArea:"",note:"",customFields:{}}]);setPreviewSummary(null);setImportMessage("");setImportError("");}}><option value="">— Chọn dự án cụ thể —</option>{data.projects.map((row)=><option key={row.id} value={row.id}>{row.code} · {row.name}</option>)}</select>}<small>{lockedProject?"Đồng bộ theo dự án đang chọn ở màn hình ngoài":data.projects.length===1?"Tự nhận theo phạm vi tài khoản":"Đang ở Tất cả dự án: có thể để trống — hệ thống dùng dự án mặc định trong phạm vi tài khoản"}</small></label>
    <label><span>Hợp đồng</span><select value={contractId} onChange={(e)=>{setContractSelection(e.target.value);setBoqVersionSelection("");setPreviewSummary(null);}}><option value="">Chọn hợp đồng</option>{contracts.map((row)=><option key={row.id} value={row.id}>{row.contractNo} · {row.contractName}</option>)}</select></label>
    <label><span>BOQ Version</span><select value={boqVersionId} onChange={(e)=>{setBoqVersionSelection(e.target.value);setPreviewSummary(null);}}><option value="">Chọn phiên bản BOQ</option>{versions.map((row)=><option key={row.id} value={row.id}>{row.versionCode} · {row.versionName||row.revisionType||"BOQ"}</option>)}</select></label>
