...
import { allocateReturnMenuItems, allocateReturnViewFor, approvalCenterGroup, approvalCenterMenuKey, configuredMenuGroups, independentMenuKeys, kpiSummaryMenuItems, legacyKpiMenuKeys, legacyReportsMenuKeys, legacySupplierPartnerMenuKeys, legacyWarehouseMenuKeys, legacyWorkMenuKeys, modules, reportsSummaryMenuItems, supplierPartnerMenuItems, supplierPartnerViewFor, warehouseMenuItems, warehouseMenuViewFor, workMenuItems } from "@/lib/menu-helpers";
...
// MT2-P9-06 (§7.6) — MÀN «CẤP PHÁT & HOÀN TRẢ VẬT TƯ»: 2 tab danh sách (Cấp phát / Hoàn trả) — nền tảng UI/list/tab/data, ⛔ không tự suy diễn nghiệp vụ (§14).
import { AllocateReturn } from "@/app/screens/AllocateReturn";
...
/**
 * MT2-P8-06 (§6.4) — NẠP danh sách VẬT TƯ THIẾU của 1 PO (vật tư PO cần nhưng NCC của PO CHƯA có).
 * ⚠️ VÌ SAO CẦN HÀM RIÊNG: `action()` **⛔ KHÔNG trả payload** (trả `undefined` khi thành công)
 *    ⇒ ⛔ KHÔNG đọc được `{ missing }`. Hàm này dùng `requestApi` (✅ CÓ trả `result`) ⇒ trả ĐÚNG mảng đó.
 * ⚠️ CHỈ ĐỌC — ⛔ KHÔNG tự thêm vật tư nào (§6.4: phải để user xác nhận).
 */
async function loadSupplierMaterialGaps(purchaseOrderId: string): Promise<Row[]> {
  if (!purchaseOrderId) return [];
  const res: Row = (await requestApi("supplier_material_gaps", { purchaseOrderId })) as Row;
  const missing = (res?.data as Row)?.missing ?? (res as Row)?.missing;
  return Array.isArray(missing) ? (missing as Row[]) : [];
}
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const normalizedData = {
        ...
        // MT2-P13-01 (§14) — luôn là MẢNG (⛔ không để undefined) để modal thông báo không vỡ khi server không trả.
        systemNotifications: Array.isArray(result.data?.systemNotifications) ? result.data.systemNotifications : [],
...
function workCenterViewFor(view: WorkMenuView | null, active: ModuleKey): WorkMenuView | null {
  ...
  // MT2-P5-01 (§3.1) — mục «Dashboard» nhóm «Công việc»: PHẢI vào tab Dashboard.
  // Trước đây mục này khai `view: "kpi"` nên rơi vào nhánh KPI bên dưới ⇒ KHÔNG khớp (`active` là
  // `work_dashboard`, không phải `dept_plan_kpi`) ⇒ trả `null` ⇒ ⛔ KHÔNG render WorkCenter (LỖI CÓ SẴN).
...
function WarehouseApp({ data, refresh, action, logout, globalError, toast }: { data: AppData; refresh: () => void; action: (name: string, payload: Row) => Promise<boolean>; logout: () => void; globalError: string; toast: string }) {
  ...
  // MT2-P9-06 (§7.6) — MỤC «Cấp phát & hoàn trả» (nhóm KHO): cùng khuôn P-07 — state `view` + children lọc
  // quyền theo `permissionKeys` (CHUNG khoá ĐÃ CÓ `warehouse_issue`); `view="list"` phân biệt màn mới với màn «Xuất».
  const [allocateReturnView, setAllocateReturnView] = useState<"list" | null>(null);
  const allocateReturnMenuChildren = allocateReturnMenuItems.flatMap((item) => {
    const viewable = item.permissionKeys.find((key) => modulePermission(data, key).canView);
    if (permissionConfigured && !viewable) return [];
    return [{ key: item.key, label: item.label, view: item.view, moduleKey: item.moduleKey, badgeKeys: item.permissionKeys }];
  });
  ...
  const [active, setActive] = useState<ModuleKey>(firstModule); const [projectSelection, setProjectSelection] = useState(initialProject); const [search, setSearch] = useState(""); const [searchOpen,setSearchOpen]=useState(false); const [modal, setModal] = useState<string | null>(data.user.mustChangePassword ? "forcePassword" : (linkedRequest ? "detail" : null)); const [selected, setSelected] = useState<Row | null>(linkedRequest); const [menuOpen, setMenuOpen] = useState(false); const [notifyOpen,setNotifyOpen]=useState(false); const [mobileNavOpen,setMobileNavOpen]=useState(false); const [requestSeed,setRequestSeed]=useState<Row | null>(null); 
  ...
  // MT2-P13-01 (§14) — modal thông báo HỆ THỐNG khi đăng nhập. ⛔ Đây là danh sách ĐÃ ĐƯỢC LỌC Ở BACKEND
  // theo userID + khoảng thời gian + chưa đọc + không bị snooze (⛔ KHÔNG lọc lại ở UI để tránh lệch quy tắc).
  // Giữ `dismissed` trong phiên để đóng modal KHÔNG làm mất thông báo khác; ⛔ KHÔNG tự đánh dấu đã đọc
  // (`mark_notification_read` là task P13-03) — chỉ «Đóng» ở P13-01.
  const [dismissedSystemNotices,setDismissedSystemNotices]=useState<string[]>([]);
  const systemNotifications=(data.systemNotifications||[]).filter((n:Row)=>!dismissedSystemNotices.includes(String(n.configId||n.id)));
  const currentSystemNotice=systemNotifications[0];
  function dismissSystemNotice(){if(currentSystemNotice)setDismissedSystemNotices((cur)=>[...cur,String(currentSystemNotice.configId||currentSystemNotice.id)]);}
  ...
  function activateModule(next:ModuleKey, view:WorkMenuView|WarehouseMenuView|SupplierPartnerMenuView|"list"|null=null){
    ...
    setWorkView(view === "personal" || view === "department" || view === "assign" || view === "kpi" || view === "reports" || view === "dashboard" ? view : null);
    setWarehouseMenuView(view === "dashboard" ? view : null);
    setSupplierPartnerView(view === "supplier" || view === "partner" ? view : null);
    // MT2-P9-06 (§7.6) — mục «Cấp phát & hoàn trả» dùng `view="list"` (cùng khuôn P-07).
  ...
  // MT2-P11-01 (§11) — ⛔ bỏ chia menu theo phòng ban cho «Báo cáo & cảnh báo»: 1 mục TỔNG HỢP dùng chung
  // 2 khoá quyền cũ. Khai báo trong code (⛔ 0 migration, ⛔ 0 khoá module mới) — đúng khuôn W-01/P-07/P9-06.
  const reportsMenuChildren = reportsSummaryMenuItems.flatMap((item) => {
    const viewable = item.permissionKeys.find((key) => modulePermission(data, key).canView);
    if (permissionConfigured && !viewable) return [];
    return [{ key: item.key, label: item.label, moduleKey: item.moduleKey, badgeKeys: item.permissionKeys }];
  });
  // MT2-P11-02 (§11) — «KPI & hiệu suất»: cũng gom về 1 mục TỔNG HỢP (⛔ không chia theo phòng ban).
  // ⚠️ Khác mảng báo cáo: mục này PHẢI mang `view` để `workCenterViewFor` mở ĐÚNG tab KPI của `WorkCenter`.
  const kpiMenuChildren = kpiSummaryMenuItems.flatMap((item) => {
    const viewable = item.permissionKeys.find((key) => modulePermission(data, key).canView);
    if (permissionConfigured && !viewable) return [];
    return [{ key: item.key, label: item.label, moduleKey: item.moduleKey, view: item.view, badgeKeys: item.permissionKeys }];
  });
  ...
  // MT2-P9-06 (§7.6) — đích đến thật của mục «Cấp phát & hoàn trả»: "list" CHỈ khi active = `warehouse_issue`.
  const allocateReturnScreenView = allocateReturnViewFor(allocateReturnView, active);
  const groupTree: Array<Row & { children: typeof allowedModules }> = menuGroups
    ...
    .map((group): Row & { children: typeof allowedModules } => ({ ...group, children: allowedModules.filter((item) => !independentMenuKeys.includes(item.key) && String(item.groupKey) === String(group.groupKey) && !legacyWorkMenuKeys.includes(item.key) && !legacyWarehouseMenuKeys.includes(item.key) && !legacySupplierPartnerMenuKeys.includes(item.key) && !legacyReportsMenuKeys.includes(item.key) && !legacyKpiMenuKeys.includes(item.key)) }))
    ...
    .filter((group) => group.children.length > 0 || (String(group.groupKey) === "my_work" && workMenuChildren.length > 0) || (String(group.groupKey) === "warehouse" && warehouseMenuChildren.length > 0) || (String(group.groupKey) === "purchasing" && supplierPartnerMenuChildren.length > 0) || (String(group.groupKey) === "reports" && (reportsMenuChildren.length > 0 || kpiMenuChildren.length > 0)));
  ...
  return <div className={`app-shell vntech-full-ui ${sidebarCollapsed?"sidebar-collapsed":""}`} data-full-release={VNTECH_BRAND.release.build} data-mobile-nav-contract="VNTECH_MOBILE_NAV_20260907" data-ui-contract={VNTECH_UI_CONTRACT_ID} data-ui-build={VNTECH_UI_BUILD_MARKER} data-functional-ui={VNTECH_FUNCTIONAL_UI_MARKER} data-regression-lock={VNTECH_RUNTIME_REGRESSION_LOCK}><aside className="sidebar vntech-app-sidebar"><div className="brand brand-logo-only" data-vntech-product={VNTECH_BRAND.productId} data-vntech-fingerprint={VNTECH_BRAND.sourceFingerprintShort}><img className="brand-logo sidebar-logo" src={VNTECH_BRAND.logoPath} alt="VNTECH TECHNOLOGY FOR LIFE" /></div><nav className="tree-nav"><button type="button" className={`nav-dashboard-direct ${active==="dashboard"?"active":""}`} onClick={()=>activateModule("dashboard")}><NavIcon name="dashboard"/><span>TỔNG QUAN ĐIỀU HÀNH</span></button>
    {groupTree.map((group) => {
      ...
      const groupBadge = group.children.reduce((sum, item) => sum + badgeFor(item.key), 0) + (groupKey==="my_work" ? workMenuChildren.reduce((sum, item) => sum + workMenuBadge(item.badgeKeys), 0) : 0) + (groupKey==="warehouse" ? warehouseMenuChildren.reduce((sum, item) => sum + warehouseMenuBadge(item.badgeKeys), 0) : 0) + (groupKey==="purchasing" ? supplierPartnerMenuChildren.reduce((sum, item) => sum + supplierPartnerMenuBadge(item.badgeKeys), 0) : 0) + (groupKey==="reports" ? reportsMenuChildren.reduce((sum, item) => sum + warehouseMenuBadge(item.badgeKeys), 0) + kpiMenuChildren.reduce((sum, item) => sum + warehouseMenuBadge(item.badgeKeys), 0) : 0);
      // MT2-P6-08 (§4.6) — «Menu chỉ có 1 item ⇒ click mở TRỰC TIẾP màn hình, ⛔ không lồng 2 cấp».
      // Nhóm `approval_center` (tên «PHÊ DUYỆT») chỉ chứa ĐÚNG một mục (`approvals` = «Trung tâm phê duyệt»)
      // ⇒ render THÀNH MỤC TRỰC TIẾP ở cấp cao nhất (cùng kiểu «TỔNG QUAN ĐIỀU HÀNH»), bỏ 1 lần bấm trung gian.
      // ⚠️ Chỉ áp dụng cho nhóm `approval_center` và CHỈ khi có đúng 1 mục — ⛔ KHÔNG áp dụng đại trà cho mọi nhóm
      // (nhóm khác có thể còn 1 mục do lọc quyền ⇒ đổi cấu trúc menu ngoài yêu cầu §4.6).
      if (groupKey === approvalCenterGroup.groupKey && group.children.length === 1) {
        const onlyChild = group.children[0];
        return <button type="button" key={groupKey} className={`nav-dashboard-direct nav-single-direct ${active === onlyChild.key ? "active" : ""}`} data-nav-single-group={groupKey} onClick={() => activateModule(onlyChild.key)}>
          <NavIcon name={onlyChild.key} kind="group"/>
      ...
      return <section className={`nav-tree-group ${childActive ? "has-active" : ""}`} data-nav-group={groupKey} data-nav-label={group.name} key={groupKey}>
        ...
        {(opened || sidebarCollapsed) && groupKey!=="material_master" && <div className="nav-children" data-nav-label={group.name}>{groupKey==="my_work"&&workMenuChildren.map((item)=>{const badge=workMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey&&workView===item.view?"active":""}`} onClick={()=>activateModule(item.moduleKey,item.view)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{groupKey==="warehouse"&&warehouseMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey&&warehouseMenuView===item.view?"active":""}`} onClick={()=>activateModule(item.moduleKey,item.view)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{groupKey==="warehouse"&&allocateReturnMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey&&allocateReturnView===item.view?"active":""}`} onClick={()=>activateModule(item.moduleKey,item.view)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{groupKey==="purchasing"&&supplierPartnerMenuChildren.map((item)=>{const badge=supplierPartnerMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey&&supplierPartnerView===item.view?"active":""}`} onClick={()=>activateModule(item.moduleKey,item.view)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{groupKey==="reports"&&reportsMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey?"active":""}`} data-vntech="reports-summary-menu" onClick={()=>activateModule(item.moduleKey)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{groupKey==="reports"&&kpiMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.moduleKey&&workView===item.view?"active":""}`} data-vntech="kpi-summary-menu" onClick={()=>activateModule(item.moduleKey,item.view)}><NavIcon name={item.moduleKey}/><span>{item.label}</span>{badge>0&&<b>{badge}</b>}</button>;})}{group.children.map((item) => { const badge = badgeFor(item.key); return <button type="button" key={item.key} className={`nav-child nav-child-${groupKey} ${active===item.key?"active":""} ${DEVELOPMENT_MODULES.has(item.key)?"is-development":""}`} onClick={() => activateModule(item.key)}><NavIcon name={item.key}/><span>{item.label}</span>{badge > 0 && <b>{badge}</b>}</button>; })}</div>}
      </section>;
    })}
  <button type="button" className="sidebar-collapse-toggle" onClick={()=>setSidebarCollapsed(value=>!value)} aria-label={sidebarCollapsed?"Mở rộng menu":"Thu gọn menu"} title={sidebarCollapsed?"Mở rộng menu":"Thu gọn menu"}><span aria-hidden="true">{sidebarCollapsed?"»":"«"}</span><b>{sidebarCollapsed?"MỞ MENU":"THU GỌN MENU"}</b></button></nav><div className="server-status" title={VNTECH_BRAND.legalOwner}><span /><div><strong>Máy chủ công ty</strong><small>Hệ thống nội bộ</small></div></div></aside>{mobileNavOpen&&<><div className="mobile-nav-backdrop" onClick={()=>setMobileNavOpen(false)} aria-hidden="true"/><div className="mobile-nav-panel mobile-nav-root" data-contract="VNTECH_MOBILE_NAV_STATES_ROOT_EXPANDED VNTECH_FULL_W2_MOBILE_NAV VNTECH_FULL_MOBILE_NAV_INTERACTION"><div className="mobile-nav-head"><div><img src={VNTECH_BRAND.logoPath} alt="VNTECH"/></div><button onClick={()=>setMobileNavOpen(false)} aria-label="Đóng menu">×</button></div><div className="mobile-nav-list mobile-nav-tree"><button type="button" className={`mobile-nav-dashboard ${active==="dashboard"?"active":""}`} aria-current={active==="dashboard"?"page":undefined} onClick={()=>{activateModule("dashboard");setMobileNavOpen(false);}}><NavIcon name="dashboard"/><span>TỔNG QUAN ĐIỀU HÀNH</span><em>›</em></button>{groupTree.filter((group)=>String(group.groupKey)!=="overview").map((group)=>{
          ...
          return <section key={groupKey} className={`mobile-nav-group ${childActive?"has-active":""}`} data-nav-action="expand"><button type="button" className="mobile-nav-parent mobile-nav-expand-only" onClick={()=>toggleGroup(groupKey)} aria-expanded={opened} aria-label={`${opened?"Thu gọn":"Mở rộng"} ${group.name}`}><NavIcon name={groupKey} kind="group"/><span>{group.name}</span>{groupBadge>0&&<b>{groupBadge}</b>}<em>{opened?"⌃":"⌄"}</em></button>{opened&&<div className="mobile-nav-children">{groupKey==="my_work"&&workMenuChildren.map((item)=>{const badge=workMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey&&workView===item.view?"active":""} aria-current={active===item.moduleKey&&workView===item.view?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey,item.view);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{groupKey==="warehouse"&&warehouseMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey&&warehouseMenuView===item.view?"active":""} aria-current={active===item.moduleKey&&warehouseMenuView===item.view?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey,item.view);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{groupKey==="warehouse"&&allocateReturnMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey&&allocateReturnView===item.view?"active":""} aria-current={active===item.moduleKey&&allocateReturnView===item.view?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey,item.view);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{groupKey==="purchasing"&&supplierPartnerMenuChildren.map((item)=>{const badge=supplierPartnerMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey&&supplierPartnerView===item.view?"active":""} aria-current={active===item.moduleKey&&supplierPartnerView===item.view?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey,item.view);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{groupKey==="reports"&&reportsMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey?"active":""} aria-current={active===item.moduleKey?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{groupKey==="reports"&&kpiMenuChildren.map((item)=>{const badge=warehouseMenuBadge(item.badgeKeys);return <button type="button" key={item.key} className={active===item.moduleKey&&workView===item.view?"active":""} aria-current={active===item.moduleKey&&workView===item.view?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.moduleKey,item.view);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}{group.children.map((item)=>{const badge=badgeFor(item.key);return <button type="button" key={item.key} className={active===item.key?"active":""} aria-current={active===item.key?"page":undefined} data-nav-action="navigate" onClick={()=>{activateModule(item.key);setMobileNavOpen(false);}}><span>{item.label}</span>{badge>0&&<b>{badge}</b>}<em aria-hidden="true">›</em></button>;})}</div>}</section>;
...
function ProjectManagement({ data, project, onProject, action, permission }: { data: AppData; project: string; onProject: (value: string) => void; open: (name: string, row?: Row) => void; action: (name: string, payload: Row) => Promise<boolean>; permission: Row }) {
  ...
  // MT2-P7-04 (§5.3) — CỔNG TẠO CÔNG VIỆC/NHIỆM VỤ cho khối chi tiết dự án.
  // TÁI DÙNG `action()` sẵn có ⇒ gửi đúng action backend `create_work_item` (backend CHẶN quyền — §17).
  // ⚠️ §5.3: CHỈ phần NHẬP DỮ LIỆU — ⛔ KHÔNG suy diễn % tiến độ (P7-05 SKIPPED).
  async function createWorkItem(payload: Row): Promise<boolean> {
    return action("create_work_item", { ...payload, projectId: payload.projectId });
  }
  ...
  return <div className="stack project-management">
    ...
    {tab === 1 && <ProjectDetailTabs data={data} project={detail} section={detailSection} onSection={setDetailSection} openEntity={openEntity} createWorkItem={createWorkItem} permission={permission} />}
    {tab === 2 && <ProjectDetailTabs data={data} project={detail} section={detailSection} onSection={setDetailSection} openEntity={openEntity} createWorkItem={createWorkItem} permission={permission} />}
    {tab === 3 && <ProjectDetailTabs data={data} project={detail} section={detailSection} onSection={setDetailSection} openEntity={openEntity} createWorkItem={createWorkItem} permission={permission} />}
    {tab === 4 && <ProjectDetailTabs data={data} project={detail} section={detailSection} onSection={setDetailSection} openEntity={openEntity} createWorkItem={createWorkItem} permission={permission} />}
...
function Approvals({ data, rows, projects, project, onProject, user, action, open, canUse, refresh }: { data: AppData; rows: Row[]; projects:Row[]; project:string; onProject:(value:string)=>void; user: Row; action: (name: string, payload: Row) => Promise<boolean>; open: (name: string, row?: Row) => void; canUse: boolean; refresh:()=>void }) {
  ...
  // MT2-P6-05 (§4.4) — QUÁ HẠN SLA ⇒ VẪN cho duyệt nhưng **BẮT BUỘC lý do**: cổng chặn TRƯỚC khi bấm
  // (backend vẫn là tầng quyết định: `RequestManagementUseCase` trả 400 nếu quá hạn + comment rỗng).
  const overdueNeedsReason=timing.late&&!approvalComment.trim();
  ...
  return <div className="stack module-screen approval-screen baseline-screen"><div className="kpi-grid approval-stage-kpis">{activeStages.map((stage,index)=><Kpi key={stage.stageNo} icon={`C${stage.stageNo}`} label={APPROVAL_STAGE_LABELS[Number(stage.stageNo)]||`CHỜ ${String(stage.name||"DUYỆT").toUpperCase()}`} value={`${format.format(stageCount(Number(stage.stageNo)))} Phiếu`} note={stage.name||`Theo hàng đợi cấp ${stage.stageNo}`} tone={index%3===1?"violet":index%3===2?"amber":undefined}/>) }<Kpi icon="QH" label="Quá hạn" value={`${format.format(overdue)} Phiếu`} note="Theo SLA của từng cấp" tone="red"/></div>
    ...
    {selected?<div className="approval-workbench baseline-approval-workbench"><section className="card approval-queue"><div className="approval-pane-head"><strong>DANH SÁCH PHIẾU CHỜ DUYỆT ({pendingRows.length})</strong><span>Sắp xếp: Mới nhất⌄</span></div><div className="approval-queue-list">{pendingRows.map((row)=>{const active=String(row.id)===String(selected.id);const ap=row.approvals?.find((item:Row)=>Number(item.stage)===Number(row.approvalStage));const tm=approvalTiming(ap);return <button key={row.id} className={active?"active":""} onClick={()=>setSelectedId(String(row.id))}><div><span className="approval-doc-icon"><NavIcon name="requests"/></span><strong>{row.requestNo}</strong><StatusBadge value={tm.late?"Khẩn cấp":row.priority==="urgent"?"Khẩn cấp":"Trung bình"}/></div><span>Dự án: {row.projectName||row.projectCode}</span><span>Khu vực: {row.area||row.purpose||"Theo phiếu đề nghị"}</span><span>Nhà cung cấp: {row.supplierName||"Chưa xác định"}</span><span>Giá trị: <b>{money(row.totalEstimatedValue)}</b></span><small>{tm.late?`Quá hạn ${durationText(tm.minutes)}`:`${durationText(tm.minutes)} trước`}</small></button>})}</div><div className="queue-pagination functional-summary"><span>{pendingRows.length} phiếu trong hàng đợi hiện tại</span></div></section>
      <section className="card approval-detail-pane"><div className="approval-detail-head"><div><small>PHIẾU ĐANG XỬ LÝ</small><h2>{selected.requestNo} {selected.priority==="urgent"&&<StatusBadge value="Khẩn cấp"/>}</h2><p>Dự án: {selected.projectName||selected.projectCode} · Nhà cung cấp: {selected.supplierName||"Chưa xác định"}</p><b>Giá trị: {money(selected.totalEstimatedValue)} · Ngày tạo: {date(selected.requestedAt)}</b></div><div className={timing.late?"sla-big late":"sla-big"}><small>SLA còn lại</small><strong>{timing.late?"QUÁ HẠN":durationText(Math.max(0,Number(currentApproval?.slaMinutes||0)-timing.minutes))}</strong><span>Hạn duyệt: {date(currentApproval?.dueAt)}</span></div></div><div className="approval-flow"><h3>QUY TRÌNH PHÊ DUYỆT (Bước {currentStage}/{approvalChainStages(data.approvalStages).filter((stage)=>stage.active).length})</h3><p className="muted" title={approvalChainWorkflow.note || undefined}>Mã luồng: <b>{approvalChainWorkflow.code}</b> · Phiên bản: <b>{approvalChainWorkflow.version}</b></p><div className="approval-flow-steps" data-vntech="approval-flow-horizontal">{approvalChainStages(data.approvalStages).filter((stage)=>stage.active).sort((a,b)=>Number(a.stageNo)-Number(b.stageNo)).map((stage)=>{const approval=selected.approvals?.find((item:Row)=>Number(item.stage)===Number(stage.stageNo));const state=approval?.status||(Number(stage.stageNo)===currentStage?"pending":"waiting");const atView=approvalDecisionAtView(approval);const commentView=approvalDecisionCommentView(approval);return <div className={`approval-flow-step ${state}`} key={stage.stageNo}><i>{stage.stageNo}</i><div><strong>{stage.name||`Bước ${stage.stageNo}`}</strong><span>{stage.description||"Theo luồng phê duyệt đã cấu hình"}</span></div><div className="approval-person"><b>{Number(stage.stageNo)>currentStage?"Đang chờ":(approval?.approverName||String(stage.allowedRoleCodes||"").split(",").filter(Boolean).map((code:string)=>roleLabel(data,code)).join(" / ")||"Chưa xác định người xử lý")}</b><small>{Number(stage.stageNo)>currentStage?"Chưa tới bước này":Number(stage.stageNo)===currentStage?"Người/nhóm đang xử lý":`Đã xử lý${approval?.department?" · "+approval.department:""}`}</small></div><em>{approval?.status==="approved"?"ĐÃ DUYỆT":Number(stage.stageNo)===currentStage?"ĐANG CHỜ":"CHỜ DUYỆT"}</em><div className="approval-step-decision" data-vntech="approval-flow-step"><span data-vntech="approval-step-decided-at"><b>Thời điểm duyệt:</b> {atView.value}{!atView.hasSource&&<small> ({atView.note})</small>}</span><span data-vntech="approval-step-comment"><b>Bình luận:</b> {commentView.hasSource?`“${commentView.value}”`:commentView.value}{!commentView.hasSource&&<small> ({commentView.note})</small>}</span></div></div>})}<div className="approval-flow-step done"><i>✓</i><div><strong>HOÀN TẤT</strong><span>Phiếu được phê duyệt</span></div><em>CHỜ XỬ LÝ</em></div></div></div>
        ...
        {permitted&&overdueNeedsReason&&<p className="approval-overdue-hint">⚠️ Bước này đã QUÁ HẠN SLA. Bắt buộc nhập lý do duyệt quá hạn vào ô BÌNH LUẬN.</p>}
        ...
        <footer className="approval-actions"><button className="secondary" onClick={()=>open("detail",selected)}>◉ CHI TIẾT</button>{permitted&&<><button className="primary" disabled={overdueNeedsReason} onClick={async()=>{if(await decide(selected.id,currentStage,"approved",action,approvalComment))setApprovalComment("");}}>✓ DUYỆT</button><button className="reject" disabled={overdueNeedsReason} onClick={async()=>{if(await decide(selected.id,currentStage,"rejected",action,approvalComment))setApprovalComment("");}}>↩ TRẢ LẠI</button><button className="supplement" onClick={()=>open("detail",selected)}>ⓘ YÊU CẦU BỔ SUNG</button></>} </footer></section>
...
function MaterialCatalogPage({ data, open, action, permission }: { data: AppData; open: (name: string, row?: Row) => void; action: (name: string, payload: Row) => Promise<boolean>; permission: Row }) {
  ...
  const [tab,setTab]=useState(0);
  // MT2-P11-04 (§12.2) — TÁCH 3 TAB **đúng nguyên văn**: [Danh sách vật tư] · [Danh mục nhóm con mã vật tư gốc] ·
  // [Mã vật tư gốc]. Trước đây dải tab là [Danh mục vật tư] · [So sánh/Đối chiếu BOQ] · [Soát trùng Alias] — 2 tab sau
  // thuộc §12.3 «⏸️ TẠM BỎ QUA» ⇒ chuyển 2 khối đó xuống khối thu gọn RIÊNG bên dưới (⛔ KHÔNG xoá chức năng).
  // Cơ chế ẩn/hiện GIỮ NGUYÊN theo CSS `canonical.css:483-486` (`data-active-tab` + `details[data-tab]`) ⇒ 0 dòng CSS mới.
  const MATERIAL_TABS=["Danh sách vật tư","Danh mục nhóm con mã vật tư gốc","Mã vật tư gốc"];
  ...
  return <div className="stack module-screen material-catalog-screen baseline-screen" data-active-tab={tab}>
    <section className="card">
      <div className="project-scope-tabs" role="tablist" aria-label="Danh mục vật tư gốc" data-vntech="material-catalog-tabs">
        {MATERIAL_TABS.map((label,i)=><button key={label} type="button" role="tab" aria-selected={tab===i} className={tab===i?"active":""} data-material-catalog-tab={String(i)} onClick={()=>setTab(i)}>{label}</button>)}
      </div>
    </section>
    <details className="module-section-collapse" open><summary><span>CÔNG CỤ CHẨN LOẠN — §12.3 «TẠM BỎ QUA» · KHÔNG nằm trong 3 tab</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body">
    <details className="module-section-collapse" open><summary><span>SO SÁNH / ĐỐI CHIẾU BOQ</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body"><MaterialMatchingWorkspace data={data} permission={permission}/></div></details>
    <details className="module-section-collapse" open><summary><span>SOÁT TRÙNG ALIAS & CHẤT LƯỢNG DANH MỤC</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body"><section className="card"><CardHead title="Soát trùng tên tương đương (alias)" note="Rà soát alias trùng normalized hoặc xung đột với tên chuẩn mã khác. Quy trình lưu/hợp nhất hiện tại đã chặn trùng khi nhập; công cụ này phát hiện dữ liệu cũ hoặc trường hợp biên chưa được chặn."/><div className="row-actions"><button className="secondary" disabled={Boolean(aliasReport)} onClick={()=>void runAliasCheck()}>{aliasReport?"Đã soát":"Bắt đầu soát trùng"}</button>{aliasReport&&<button className="secondary" onClick={()=>setAliasReport(null)}>Đóng kết quả</button>}</div>
    ...
    <details className="module-section-collapse" data-tab="0" open><summary><span>DANH SÁCH VẬT TƯ</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body"><MaterialListTable data={data} open={open} permission={permission}/></div></details>
    <details className="module-section-collapse" data-tab="1"><summary><span>DANH MỤC NHÓM CON MÃ VẬT TƯ GỐC</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body">{permission.canEdit?<MaterialCatalogManager data={data} open={open} action={action} section="subgroups"/>:<section className="card"><Empty text="Tài khoản chỉ có quyền xem — nhóm con mã vật tư gốc do quản trị hệ thống duy trì."/></section>}</div></details>
    <details className="module-section-collapse" data-tab="2"><summary><span>MÃ VẬT TƯ GỐC</span><b>Ẩn / Hiện</b></summary><div className="module-section-collapse-body">{permission.canEdit?<MaterialCatalogManager data={data} open={open} action={action} section="masters"/>:<section className="card"><CardHead title="Mã vật tư gốc" note="Tài khoản chỉ có quyền xem"/><DataTable rows={materials.filter((row)=>row.active!==0)} rowKey={(row)=>String(row.id)} emptyText="Danh mục vật tư chưa có dữ liệu." columns={[{key:"c1",header:"Mã vật tư",render:(row)=><strong className="code">{row.code}</strong>},{key:"c2",header:"Tên chuẩn",render:(row)=><>{row.name}</>},{key:"c3",header:"Hệ M&E",render:(row)=><>{row.categoryName||row.system||"—"}</>},{key:"c4",header:"Nhóm",render:(row)=><>{sanitizeUiText(row.subcategoryName||"—")||"—"}</>},{key:"c5",header:"ĐVT",render:(row)=><>{row.unit||"—"}</>},{key:"c6",header:"Thông số",render:(row)=><>{sanitizeUiText(row.specification||"—")||"—"}</>},{key:"c7",header:"Trạng thái",render:()=><StatusBadge value="Đang dùng"/>}]}/></section>}</div></details>
  </div>;
}
...
// MT2-P11-04 (§12.2) — `section` chọn KHỐI nào hiện: "subgroups" = Danh mục nhóm con mã vật tư gốc · "masters" = Mã vật tư gốc
// (mặc định "all" giữ nguyên hành vi cũ khi gọi không truyền prop) ⇒ 2 danh sách TÁCH RIÊNG theo yêu cầu
// «mỗi tab hiển thị danh sách tương ứng», ⛔ không nhân bản component/khối dữ liệu.
function MaterialCatalogManager({ data, open, action, section = "all" }: { data: AppData; open: (name: string, row?: Row) => void; action: (name: string, payload: Row) => Promise<boolean>; section?: "subgroups" | "masters" | "all" }) {
  ...
  return <div className="stack material-master-workspace" data-material-master="true" data-material-section={section}>
    {(section==="all"||section==="subgroups")&&<section className="card material-subgroup-reference"><div className="material-subgroup-head"><div><h2>DANH MỤC NHÓM CON MÃ VẬT TƯ GỐC</h2><p>Hệ M&E → Nhóm vật tư → Mã vật tư gốc</p></div><div className="material-subgroup-kpis"><article><span>▣</span><small>Tổng hệ</small><strong>{categories.filter(r=>Number(r.active)!==0).length}</strong></article><article><span>◇</span><small>Nhóm con đề xuất</small><strong>{subcategories.filter(r=>Number(r.active)!==0).length}</strong></article><article><span>◷</span><small>Chờ duyệt</small><strong>{proposed}</strong></article></div></div>
    ...
    <div className="table-wrap"><table className="baseline-table staff-full-table">
      ...
      <tbody>
        {shown.map((u, i) => <tr key={String(u.id)} className="account-row" onClick={() => openEntity("user", u)} title="Bấm để mở chi tiết tài khoản (EntityDetailModal)">
          ...
          <td>{u.employeeCode ? <strong className="code">{u.employeeCode}</strong> : <span className="muted" data-no-employee-code="true" title="Tài khoản chưa có mã nhân viên — cần gán mã để khớp với Hồ sơ nhân sự (§13.3)">Chưa có mã</span>}</td>
          <td>{u.username || "—"}</td>
          ...
          {/* MT2-P12-03 (§13.3) — ⛔ bỏ ô «Hạn mức»: entry tương ứng đã gỡ khỏi `ACCOUNT_COLUMNS` nên cột biến mất
              (tiêu đề sinh từ cùng mảng ⇒ không thể lệch số cột). ⛔ CSDL `users.approval_limit` GIỮ NGUYÊN. */}
...
// ─────────────────────────────────────────────────────────────────────────────
// MT2-P12-02 (§13.2) — MODAL TẠO / SỬA CẤU HÌNH THÔNG BÁO.
...
//
// ⚠️ KHUÔN REUSE: `BaseModal` + `ModalFooter` + `FormEvent` đúng như `SystemLevelModal` (⛔ không dựng modal thứ hai).
// ⚠️ HỢP ĐỒNG BACKEND (`NotificationManagementUseCase.saveConfig:114-157`):
//    · Update quyết định bằng **`configId`** (⛔ không phải `id`) · `channel ∈ {web,email}` ·
//      `recipientMode ∈ {all,user,users,department,project}` (400 nếu sai) · `targets:[{targetType,targetId}]`
//      được **THAY TOÀN BỘ** (⛔ không trộn cũ/mới) · thời gian nhận ISO-8601 **hoặc** `yyyy-MM-ddTHH:mm` quy về **UTC**
//      (`instantOrNull:245-258`) ⇒ gửi `toISOString()` để khỏi lệch múi giờ.
//    · `endAt` ⛔ chỉ có ý nghĩa với **Web** (§13.2) ⇒ ẩn + gửi rỗng khi kênh Email.
// ─────────────────────────────────────────────────────────────────────────────
const NOTIFICATION_TARGET_TYPES: Record<string, string> = {
  user: "user",
  users: "user",
...
function NotificationConfigModal({ data, row, close, submit }: { data: AppData; row?: Row; close: () => void; submit: (name: string, payload: Row) => Promise<boolean> }) {
  const editing = Boolean(row?.id);
  const [channel, setChannel] = useState(String(row?.channel || "web"));
  const [code, setCode] = useState(String(row?.code || ""));
  const [name, setName] = useState(String(row?.name || ""));
  const [content, setContent] = useState(String(row?.content || ""));
  const [recipientMode, setRecipientMode] = useState(String(row?.recipientMode || "all"));
  const [sendAt, setSendAt] = useState(String(row?.sendAt || "").slice(0, 16));
  const [endAt, setEndAt] = useState(String(row?.endAt || "").slice(0, 16));
  const [targetIds, setTargetIds] = useState<string[]>([]);
  const [error, setError] = useState("");

  const options = recipientMode === "department"
    ? (data.organizationUnits || []).filter((unit) => Number(unit.active) !== 0).map((unit) => ({ id: String(unit.id), label: `${unit.code || "—"} · ${unit.name || "—"}` }))
    : recipientMode === "project"
      ? (data.adminProjects || []).map((project) => ({ id: String(project.id), label: `${project.code || "—"} · ${project.name || "—"}` }))
      : (data.users || []).filter((user) => Number(user.active) !== 0).map((user) => ({ id: String(user.id), label: `${user.fullName || user.username || "—"} · ${user.username || "—"}` }));

  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setError("");
    if (!name.trim()) return setError("Nhập Tên thông báo.");
    if (!code.trim()) return setError("Nhập Mã thông báo.");
    if (recipientMode !== "all" && targetIds.length === 0) return setError("Chọn ít nhất 1 đối tượng nhận thông báo.");
    const targetType = NOTIFICATION_TARGET_TYPES[recipientMode] || "user";
    const ok = await submit("save_notification_config", {
      ...(editing ? { configId: row?.id } : {}),
      code: code.trim(),
      name: name.trim(),
      channel,
      content: content.trim(),
      recipientMode,
      sendAt: toUtcIso(sendAt),
      endAt: channel === "web" ? toUtcIso(endAt) : "",
      targets: recipientMode === "all" ? [] : targetIds.map((targetId) => ({ targetType, targetId })),
    });
    if (ok) close();
  }
  ...
  return <BaseModal title={editing ? `Sửa thông báo ${row?.code || ""}` : "Tạo cấu hình thông báo"} note="Cấu hình dùng để gửi thông báo qua Web hoặc Email. Người nhận có thể chọn 1 user, nhiều user, phòng ban, dự án hoặc toàn bộ user." close={close}>
    <form onSubmit={save} data-vntech="notification-config-modal">
      <div className="modal-body"><div className="form-grid">
        <label><span>Loại thông báo *</span><select required value={channel} onChange={(event) => setChannel(event.target.value)}><option value="web">Web</option><option value="email">Email</option></select></label>
        <label><span>Mã thông báo *</span><input required value={code} onChange={(event) => setCode(event.target.value)}/></label>
        <label className="span-2"><span>Tên thông báo *</span><input required value={name} onChange={(event) => setName(event.target.value)}/></label>
        <label className="span-2"><span>Nội dung</span><textarea rows={3} value={content} onChange={(event) => setContent(event.target.value)}/></label>
        <label><span>Thời gian gửi</span><input type="datetime-local" value={sendAt} onChange={(event) => setSendAt(event.target.value)}/><small>Để trống = gửi ngay khi bật.</small></label>
        {channel === "web" && <label><span>Thời gian kết thúc</span><input type="datetime-local" value={endAt} onChange={(event) => setEndAt(event.target.value)}/><small>Chỉ áp dụng cho thông báo Web.</small></label>}
        <label className="span-2"><span>Người nhận *</span><select value={recipientMode} onChange={(event) => { setRecipientMode(event.target.value); setTargetIds([]); }}><option value="all">Toàn bộ user</option><option value="user">1 user</option><option value="users">Nhiều user</option><option value="department">Phòng ban</option><option value="project">Dự án</option></select></label>
      </div>
      {recipientMode !== "all" && <fieldset className="notification-target-picker"><legend>Chọn đối tượng nhận ({targetIds.length} đã chọn)</legend><div className="notification-target-list">{options.map((option) => <label key={option.id}><input type="checkbox" checked={targetIds.includes(option.id)} onChange={() => toggleTarget(option.id)}/><span>{option.label}</span></label>)}</div></fieldset>}
      {error && <div className="inline-alert">{error}</div>}
      </div>
      <ModalFooter close={close} label={editing ? "Lưu thay đổi" : "Tạo thông báo"}/>
    </form>
  </BaseModal>;
}
...
function Admin({ data, open, action }: { data: AppData; open: (name: string, row?: Row) => void; action: (name: string, payload: Row) => Promise<boolean> }) {
  ...
  // MT2-P12-01 (§13.1) — TAB THÔNG BÁO: danh sách cấu hình (R) + bật/tắt + xoá (D) + Search/Sort/Filter.
  // ⚠️ CHỮ C + U của CRUD (Tạo / Sửa) = modal `save_notification_config` ⇒ **task P12-02**; ⛔ KHÔNG dựng nút chết.
  // ⚠️ Nguồn: `notification_configs` (action chỉ đọc, RBAC module `admin`) trả `{ok, configs:[…]}` — ⛔ KHÔNG có
  //    trong bootstrap ⇒ phải nạp bằng `requestApi` (⛔ `action()` trả `undefined` nên không đọc được payload).
  const [notifConfigs,setNotifConfigs]=useState<Row[]>([]);
  ...
  const NOTIF_CHANNEL_LABEL:Record<string,string>={web:"Web",email:"Email"};
  const NOTIF_RECIPIENT_LABEL:Record<string,string>={all:"Toàn bộ user",user:"1 user",users:"Nhiều user",department:"Phòng ban",project:"Dự án"};
  async function loadNotifConfigs(){
    setNotifError("");
    try{ const res=await requestApi("notification_configs"); setNotifConfigs((res?.configs||[]) as Row[]); }
    catch(error){ setNotifConfigs([]); setNotifError(error instanceof Error?error.message:"Không tải được danh sách cấu hình thông báo."); }
  }
  // ⚠️ MT2-P14-03c — SỬA LỖI `lint` (rule «Calling setState synchronously within an effect»): bản cũ gọi
  // `void loadNotifConfigs()` trong effect, mà hàm đó `setNotifError("")` **đồng bộ TRƯỚC** `await` ⇒ eslint báo
  // error ⇒ `npm test` (lint → typecheck → regression → workflow) ĐỎ. Nay effect tự chạy async, mọi `setState`
  // nằm SAU mốc `await` và có cờ `live` chống cập nhật khi unmount/đổi bước. ⛔ Giữ nguyên `loadNotifConfigs`
  // cho nút «↻ Tải lại» và sau thao tác Bật/Tắt/Xoá (gọi từ sự kiện, ⛔ không trong effect).
  useEffect(()=>{
    if(step!==13)return;
    let live=true;
    void (async()=>{
      try{ const res=await requestApi("notification_configs"); if(live){ setNotifConfigs((res?.configs||[]) as Row[]); setNotifError(""); } }
      catch(error){ if(live){ setNotifConfigs([]); setNotifError(error instanceof Error?error.message:"Không tải được danh sách cấu hình thông báo."); } }
    })();
    return ()=>{ live=false; };
  },[step]);
  const notifRows=notifConfigs
    .filter(row=>notifChannel==="ALL"||String(row.channel||"")===notifChannel)
    .filter(row=>notifStatus==="ALL"||(notifStatus==="ACTIVE"?Number(row.active)!==0:Number(row.active)===0))
    .filter(row=>{ if(!notifQuery.trim())return true; const hay=`${row.code||""} ${row.name||""} ${row.content||""}`.toLocaleLowerCase("vi"); return hay.includes(notifQuery.trim().toLocaleLowerCase("vi")); })
    .sort((a,b)=>{ if(notifSort==="oldest")return String(a.createdAt||"").localeCompare(String(b.createdAt||"")); if(notifSort==="name")return String(a.name||"").localeCompare(String(b.name||""),"vi"); return String(b.createdAt||"").localeCompare(String(a.createdAt||"")); });
  ...
  return <div className="stack admin-approved-screen baseline-screen">
    ...
    {step===13&&<section className="card notification-config-admin" data-vntech="notification-config-tab"><ListToolbar
      ...
      total={notifConfigs.length}
      unit="cấu hình"
      search={{value:notifQuery,onChange:setNotifQuery,placeholder:"Tìm mã · tên · nội dung"}}
      filters={[{key:"channel",label:"Kênh",value:notifChannel,onChange:setNotifChannel,options:[{value:"ALL",label:"Tất cả kênh"},{value:"web",label:"Web"},{value:"email",label:"Email"}]},{key:"status",label:"Trạng thái",value:notifStatus,onChange:setNotifStatus,options:[{value:"ALL",label:"Tất cả"},{value:"ACTIVE",label:"Đang bật"},{value:"INACTIVE",label:"Đã tắt"}]}]}
      sort={{value:notifSort,onChange:setNotifSort,options:[{value:"newest",label:"Mới nhất"},{value:"oldest",label:"Cũ nhất"},{value:"name",label:"Tên A→Z"}]}}
      actions={<><button type="button" className="secondary" onClick={()=>void loadNotifConfigs()}>↻ Tải lại</button><button type="button" className="primary" onClick={()=>open("notificationConfig")}>＋ TẠO THÔNG BÁO</button></>}
    />
    {notifError&&<div className="inline-alert">{notifError}</div>}
    <div className="table-wrap"><table><thead><tr><th>Mã</th><th>Tên</th><th>Kênh</th><th>Người nhận</th><th>Nội dung</th><th>Thời gian</th><th>Trạng thái</th><th></th></tr></thead><tbody>
      {notifRows.map(row=><tr key={String(row.id)}><td><strong className="code">{row.code||"—"}</strong></td><td>{row.name||"—"}</td><td>{NOTIF_CHANNEL_LABEL[String(row.channel||"")]||row.channel||"—"}</td><td>{NOTIF_RECIPIENT_LABEL[String(row.recipientMode||"all")]||row.recipientMode||"—"}</td><td>{String(row.content||"—").slice(0,90)}{String(row.content||"").length>90?"…":""}</td><td>{row.sendAt?date(row.sendAt):"Ngay"}{row.endAt?<small>Đến {date(row.endAt)}</small>:null}</td><td><StatusBadge value={Number(row.active)===0?"Đã tắt":"Đang bật"}/></td><td><div className="row-actions"><button type="button" className="export-mini" onClick={()=>open("notificationConfig",row)}>Sửa</button><button type="button" className="export-mini" onClick={()=>void action("set_notification_config_status",{configId:row.id,active:Number(row.active)===0?1:0}).then(()=>loadNotifConfigs())}>{Number(row.active)===0?"Bật":"Tắt"}</button><button type="button" className="export-mini danger" onClick={()=>{ if(!window.confirm(`Xoá cấu hình thông báo ${row.code}? Lịch sử đã đọc của từng user vẫn được giữ.`))return; void action("delete_notification_config",{configId:row.id}).then(()=>loadNotifConfigs()); }}>Xoá</button></div></td></tr>)}
      {!notifRows.length&&<tr><td colSpan={8}><Empty text="Chưa có cấu hình thông báo nào."/></td></tr>}
    </tbody></table></div>
    </section>}
...
function RequestModal({ data, contextProject, close, submit, initialMaterial }: { data: AppData; contextProject:string; close: () => void; submit: (name: string, payload: Row) => Promise<boolean>; /** MT2-P8-08 (§6.7) — TỰ FILL khi lập PR từ CARD «Vật tư đang thiếu». ⚠️ Dòng này đến từ `inventory` (materialId/materialCode/... + minStock/balance/projectId), ⛔ KHÔNG phải từ `materials` ⇒ phải chấp nhận CẢ HAI hình dạng. ⚠️ TÙY CHỌN ⇒ nơi gọi cũ vẫn hợp lệ. */ initialMaterial?: Row }) {
  const firstMaterial = data.materials[0];
  // §6.7 ⑤ — TỰ FILL: `material` · `quantity` · `project`. ⚠️ `contract`/`BOQ` TỰ DẪN XUẤT từ `projectId` (bên dưới).
  ...
  const [lines, setLines] = useState<Row[]>([{ materialId:seed?.id??seed?.materialId,materialCode:seed?.code??seed?.materialCode,materialName:seed?.name??seed?.materialName,unit:seed?.unit,quantity:initialMaterial?Math.max(1,seedShortage):1,unitPrice:seed?.standardPrice||0,boqItemId:"",contractLineNo:"",origin:"",approvedSupplier:"",installationArea:"",note:"",customFields:{} }]);
  const lockedProject=contextProject!=="ALL"&&Boolean(contextProject); const initialRequestProject=initialMaterial?.projectId?String(initialMaterial.projectId):(lockedProject?String(contextProject):(data.projects.length===1?String(data.projects[0]?.id||""):""));
  ...
  // TASK-139 (1) — «người dùng đã tự chọn»: CHƯA chạm ⇒ giữ mặc định như cũ (không đổi UX);
  // ĐÃ chạm ⇒ tôn trọng ĐÚNG lựa chọn, kể cả "— Tùy chọn (để trống) —" (giá trị "") ⇒ chọn trống được thật.
  const [contractTouched,setContractTouched]=useState(false); const [boqTouched,setBoqTouched]=useState(false);
  ...
  const contractId=contracts.some((row)=>String(row.id)===String(contractSelection))?String(contractSelection):(contractTouched?"":String(defaultContract?.id||""));
  ...
  const boqVersionId=versions.some((row)=>String(row.id)===String(boqVersionSelection))?String(boqVersionSelection):(boqTouched?"":String(defaultVersion?.id||""));
  ...
  async function send(event:FormEvent<HTMLFormElement>){event.preventDefault();if(lines.length<1||lines.length>100){setImportError("Phiếu phải có từ 1 đến 100 dòng vật tư.");return;}const unresolved=lines.filter((row)=>row.matchStatus&&row.matchStatus!=="exact"&&row.materialSource!=="catalog");if(previewSummary&&unresolved.length){setImportError(`Còn ${unresolved.length} dòng chưa xác định đúng BOQ. Hãy chọn/map đúng dòng trước khi lập phiếu.`);return;}const payload=Object.fromEntries(new FormData(event.currentTarget));if(await submit("create_request",{...payload,projectId,contractId,boqVersionId,lines}))close();}
  ...
  return <BaseModal title="Lập đề nghị cấp vật tư" note="Có thể chọn Dự án → Hợp đồng → BOQ Version để đối chiếu khối lượng (không bắt buộc). File chỉ nhập nhu cầu; hệ thống tự lấy khối lượng HĐ, lũy kế, tồn kho và hàng đang chờ giao. Sau khi gửi, phiếu vào luồng phê duyệt ngay và không tự thu hồi." close={close}><form className="request-form" onSubmit={send}><div className="modal-body"><div className="request-company-head"><div><img src={VNTECH_BRAND.logoPath} alt="VNTECH"/><strong>ĐỀ NGHỊ CẤP VẬT TƯ</strong></div><p>Kính gửi: <b>GIÁM ĐỐC CÔNG TY, CÁC PHÒNG/BAN CÔNG TY</b></p><small>Số phiếu tự động: {`DNMH-${project?.code||"DAxx"}-${new Date().getFullYear()}-xxxx`}</small></div><div className="form-grid request-context-grid">
    <label><span>{label("projectId","Dự án")}</span>{lockedProject?<strong className="locked-project-value">{project?.code} · {project?.name}</strong>:<select name="projectId" value={projectId} onChange={(e)=>{setProjectId(e.target.value);setContractSelection("");setBoqVersionSelection("");setContractTouched(false);setBoqTouched(false);setLines([{materialId:"",materialCode:"",materialName:"",unit:"",quantity:1,unitPrice:0,boqItemId:"",contractLineNo:"",origin:"",approvedSupplier:"",installationArea:"",note:"",customFields:{}}]);setPreviewSummary(null);setImportMessage("");setImportError("");}}><option value="">— Tùy chọn (để trống) —</option>{data.projects.map((row)=><option key={row.id} value={row.id}>{row.code} · {row.name}</option>)}</select>}<small>{lockedProject?"Đồng bộ theo dự án đang chọn ở màn hình ngoài":"Không bắt buộc: có thể để trống — phiếu sẽ không thuộc dự án nào"}</small></label>
    <label><span>Hợp đồng</span><select value={contractId} onChange={(e)=>{setContractTouched(true);setBoqTouched(false);setContractSelection(e.target.value);setBoqVersionSelection("");setPreviewSummary(null);}}><option value="">— Tùy chọn (để trống) —</option>{contracts.map((row)=><option key={row.id} value={row.id}>{row.contractNo} · {row.contractName}</option>)}</select></label>
    <label><span>BOQ Version</span><select value={boqVersionId} onChange={(e)=>{setBoqTouched(true);setBoqVersionSelection(e.target.value);setPreviewSummary(null);}}><option value="">— Tùy chọn (để trống) —</option>{versions.map((row)=><option key={row.id} value={row.id}>{row.versionCode} · {row.versionName||row.revisionType||"BOQ"}</option>)}</select></label>
...
function LineEditor({ data, projectId, contractId, boqVersionId, lines, setLines }: { data: AppData; projectId: string; contractId:string; boqVersionId:string; lines: Row[]; setLines: (rows: Row[]) => void }) {
  ...
  // TASK-139 (2) — ô «tên vật tư» tìm trong DANH MỤC VẬT TƯ GỐC (`data.materials`), KHÔNG còn tìm theo BOQ.
  // MỘT datalist dùng chung cho mọi dòng (danh mục gốc lớn hơn BOQ dự án ⇒ tránh nhân bản DOM theo số dòng).
  const materialSearchListId="request-material-catalog-search";
...
function ReceiptModal({ data, close, submit }: { data: AppData; close: () => void; submit: (name: string, payload: Row) => Promise<boolean> }) {
  ...
  // MT2-P9-04 (§7.4) — «tạo phiếu nhập TỪ STO/phiếu xuất kho». ⚠️ CHỈ `transferOrders` (STO) CÓ kho đi/kho đến
  // (`source_warehouse_id`/`destination_warehouse_id`); `stock_issues` CHỈ khai project+team ⇒ ⛔ KHÔNG bịa kho (§14).
  const [sourceType, setSourceType] = useState("PO");
  const [transferId, setTransferId] = useState("");
  const [issueId, setIssueId] = useState("");
  const relatedTransfer = (data.transferOrders || []).find((row) => String(row.id) === String(transferId)) || null;
  const relatedIssue = (data.issues || []).find((row) => String(row.id) === String(issueId)) || null;
  ...
  return <BaseModal title="Ghi nhận số lượng giao thực tế" note="Có thể nhập thừa hoặc thiếu so với PO; hệ thống chỉ chấp nhận tối đa số đã đặt và lưu riêng phần chênh lệch" close={close}><form className="receipt-form" onSubmit={send}><div className="modal-body"><div className="form-grid"><label><span>Đơn đặt hàng *</span><select value={poId} onChange={(event) => setPoId(event.target.value)} required><option value="">Chọn PO</option>{openPos.map((row) => <option key={row.id} value={row.id}>{row.poNo} · ETA {date(row.eta)} · {row.supplierName}</option>)}</select></label><label><span>Số phiếu giao hàng</span><input name="deliveryNoteNo" placeholder="Số phiếu của nhà cung cấp" /></label><label><span>Chứng chỉ / CO-CQ</span><select name="certificateStatus" defaultValue="complete"><option value="complete">Đã có</option><option value="missing">Chưa có</option><option value="not_required">Không yêu cầu</option></select></label><label><span>Giấy giao hàng kèm theo</span><select name="deliveryDocumentStatus" defaultValue="complete"><option value="complete">Đã có</option><option value="missing">Chưa có</option></select></label></div>{po?.items?.length ? <div className="line-editor"><div className="delivery-head"><span>Vật tư</span><span>PO / Đã giao thực tế</span><span>Thực giao lần này</span><span>Lô / serial</span></div>{po.items.filter((item: Row) => Number(item.actualDeliveredQty) + Number(item.closedQty) < Number(item.orderedQty)).map((item: Row) => <div className="delivery-line" key={item.id}><div><strong>{item.materialCode}</strong><small>{item.materialName} · {item.unit}</small></div><span>{format.format(item.orderedQty)} / {format.format(item.actualDeliveredQty)}</span><input name={`qty-${item.id}`} type="number" min="0" step="0.001" defaultValue={Math.max(0, Number(item.orderedQty) - Number(item.actualDeliveredQty) - Number(item.closedQty))} /><input name={`lot-${item.id}`} placeholder="Lô / serial" /></div>)}</div> : <Empty text="PO chưa có dòng có thể nhận." />}<div className="check-grid"><label><input type="checkbox" name="qcOk" defaultChecked /> <span><b>QC thực tế đạt</b><small>Đúng quy cách, hãng và tình trạng hàng</small></span></label><div className="form-grid" data-vntech="receipt-source-picker"><label><span>Nguồn phiếu *</span><select value={sourceType} onChange={(e)=>setSourceType(e.target.value)} data-vntech="receipt-source-type"><option value="PO">Đơn mua (PO)</option><option value="STO">Phiếu điều chuyển kho (STO)</option><option value="ISSUE">Phiếu xuất kho</option></select></label>{sourceType==="STO"&&<label><span>Phiếu điều chuyển *</span><select value={transferId} onChange={(e)=>setTransferId(e.target.value)} data-vntech="receipt-source-sto"><option value="">— Chọn STO —</option>{(data.transferOrders||[]).slice(0,50).map((row)=><option key={row.id} value={String(row.id)}>{row.transferNo} · {row.sourceWarehouseName} → {row.destinationWarehouseName}</option>)}</select></label>}{sourceType==="ISSUE"&&<label><span>Phiếu xuất kho *</span><select value={issueId} onChange={(e)=>setIssueId(e.target.value)} data-vntech="receipt-source-issue"><option value="">— Chọn phiếu xuất —</option>{(data.issues||[]).slice(0,50).map((row)=><option key={row.id} value={String(row.id)}>{row.issueNo} · {row.projectCode} · {row.teamName}</option>)}</select></label>}</div>
      {relatedTransfer&&<section className="drawer-section" data-vntech="receipt-related-sto"><CardHead title="Thông tin phiếu điều chuyển liên quan" note="§7.4 — tự fill kho đi/kho đến từ STO; nếu phiếu chưa có kho thì người dùng tự nhập"/><div className="summary-grid request-summary"><div><small>Mã STO</small><strong>{relatedTransfer.transferNo}</strong></div><div><small>Kho đi</small><strong>{relatedTransfer.sourceWarehouseName} ({relatedTransfer.sourceWarehouseCode})</strong></div><div><small>Kho đến</small><strong>{relatedTransfer.destinationWarehouseName} ({relatedTransfer.destinationWarehouseCode})</strong></div><div><small>Trạng thái</small><StatusBadge value={statusLabel(relatedTransfer)}/></div><div><small>Ngày yêu cầu</small><strong>{date(relatedTransfer.requestedAt)}</strong></div><div><small>Số dòng</small><strong>{format.format(Number(relatedTransfer.itemCount||0))} dòng</strong></div><div><small>Yêu cầu / Đã xuất / Đã nhận</small><strong>{format.format(Number(relatedTransfer.requestedQty||0))} / {format.format(Number(relatedTransfer.shippedQty||0))} / {format.format(Number(relatedTransfer.receivedQty||0))}</strong></div></div></section>}
      {relatedIssue&&<section className="drawer-section" data-vntech="receipt-related-issue"><CardHead title="Thông tin phiếu xuất kho liên quan" note="§7.4 — stock_issues chỉ khai dự án + tổ đội (KHÔNG có kho) ⇒ không hiển thị kho để tránh bịa dữ liệu"/><div className="summary-grid request-summary"><div><small>Mã phiếu xuất</small><strong>{relatedIssue.issueNo}</strong></div><div><small>Dự án</small><strong>{relatedIssue.projectCode}</strong></div><div><small>Tổ đội nhận</small><strong>{relatedIssue.teamName}</strong></div><div><small>Người nhận</small><strong>{relatedIssue.receivedByName||"—"}</strong></div><div><small>Trạng thái</small><StatusBadge value={statusLabel(relatedIssue)}/></div><div><small>Ngày xuất</small><strong>{date(relatedIssue.issuedAt)}</strong></div></div></section>}
      <div className="flow-hint"><b>Bước tiếp theo</b><span>Lưu chuyến giao → tải ảnh → BCH xác nhận → chuyển Đơn hàng đã giao</span></div></div></div><ModalFooter close={close} label="Lưu thực giao và chuyển BCH xác nhận →" disabled={!po} /></form></BaseModal>;
}
...
// MT2-P10-02 (§10.1) — Modal «Hồ sô nhân sự chi detalle» PHẢI CÓ 3 TAB (nguyên văn):
// [Thông tin user] (tab đầu) · [Thông tin cá nhắn] · [Dự án đã và đang tham gia].
// Khối «Thao tác gần đây» + «Đơn từ & giấy tờ» thuộc phạm vi THÔNG THÌN USER (tài khoản) ⇒
// đặt at tab 0 để ⛔ KHÔNG bỏ dữ liệu hiện có (§39/§40 — no scope creep, no data loss).
const USER_PROFILE_TABS = ["Thông tin user", "Thông tin cá nhắn", "Dự án đã và đang tham gia"] as const;
function UserProfilePanel({ data, row, close, open, showDocuments = false }: { data: AppData; row: Row; close: () => void; open: (name: string, r?: Row) => void; showDocuments?: boolean }) {
  ...
  // MT2-P10-02 — tab activ hiện tại (0 = Thông tin user — §10.1 tab đầu).
  ...
  return <BaseModal title="Hồ sơ nhân sự chi tiết" note="Chức vụ · phòng ban · liên hệ · cá nhân · dự án đã và đang tham gia." close={close}>
    <div className="modal-body">
      <div className="project-scope-tabs" role="tablist" aria-label="user-profile-tabs" data-vntech="user-profile-tabs">{USER_PROFILE_TABS.map((label, index) => <button type="button" key={label} role="tab" aria-selected={profileTab === index} className={profileTab === index ? "active" : ""} data-user-profile-tab={String(index)} onClick={() => setProfileTab(index)}>{label}</button>)}</div>
      {profileTab === 0 && <div className="stack">
      <section className="card">
      ...
      {profileTab === 1 && <div className="stack">
      <section className="card">
      ...
      {profileTab === 2 && <div className="stack">
      <section className="card">
...
function UserEditModal({ data, row, close, submit }: { data: AppData; row: Row; close: () => void; submit: (name: string, payload: Row) => Promise<boolean> }) {
  ...
  // MT2-P12-06 (§13.4) — chữ ký: prefill từ chữ ký ĐANG CÓ (⛔ không làm mất khi mở modal sửa);
  // gửi kèm `signatureUrl` ⇒ chọn ảnh mới GHI ĐÈ ảnh cũ, bấm «Xóa chữ ký» gửi rỗng ⇒ backend ghi NULL.
  const [signature,setSignature]=useState(String(row.signatureUrl||""));
  ...
  async function send(event: FormEvent<HTMLFormElement>) {
    ...
    const updated=await submit("update_user", { ...Object.fromEntries(form), userId: row.id, active: Boolean(row.active), signatureUrl: signature }); if(!updated)return;
  ...
  return <BaseModal title={`Sửa tài khoản · ${row.fullName}`} note="Thông tin tài khoản và ma trận quyền dùng chung đúng một nguồn dữ liệu với Ngoại lệ cá nhân; sửa ở đây sẽ phản ánh tại màn hình phân quyền." close={close}><form onSubmit={send}><div className="modal-body"><div className="form-grid"><label><span>Mã nhân viên *</span><input name="employeeCode" required defaultValue={row.employeeCode || ""} /></label><label><span>Họ tên *</span><input name="fullName" required defaultValue={row.fullName || ""} /></label><label><span>Tên đăng nhập *</span><input name="username" required defaultValue={row.username || ""} /></label><label><span>Email công ty</span><input name="email" type="email" defaultValue={row.email || ""} /></label><label><span>Vai trò *</span><select name="role" defaultValue={row.role}>{roles.map((role) => <option key={role.code} value={role.code}>{role.name}</option>)}</select></label><label><span>Phòng / Bộ phận *</span><select name="organizationUnitId" required defaultValue={row.organizationUnitId||""}><option value="" disabled>Chọn đơn vị</option>{organizations.map((item)=><option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}</select></label><label><span>Mật khẩu mới (nếu đặt lại)</span><input name="newPassword" type="password" minLength={8} pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}" title="≥8 ký tự, hoa, thường, số, ký tự đặc biệt" placeholder="Để trống nếu không đổi" /></label></div><div className="signature-field-slot"><SignatureField value={signature} onChange={setSignature}/></div>
...
// ─────────────────────────────────────────────────────────────────────────────
// MT2-P12-06 (§13.4) — Ô CHỮ KÝ dùng chung cho modal TẠO và modal CHỈNH SỬA tài khoản.
...
//
// ⚠️ KHUÔN REUSE: kiểm MIME + giới hạn 2 MB + `FileReader.readAsDataURL` **Y HỆT** `pickAvatar`
// (`AccountSettingsModal`) ⇒ ⛔ không phát minh luật ảnh thứ hai (§15). Backend nhận data-URI qua
// khoá `signatureUrl` (`UserManagementUseCase.updateUser` từ P3-04 + `createUser` từ P12-06).
// 📦 Lưu 1 chuỗi ⇒ **chỉ 1 ảnh** theo đúng dữ liệu (`users.signature_url` là cột đơn) — không có danh sách
// ⇒ ⛔ không thể tồn tại 2 chữ ký active; chọn ảnh mới **ghi đè** giá trị cũ (xoá/thay ảnh cũ).
// ─────────────────────────────────────────────────────────────────────────────
function SignatureField({ value, onChange }: { value: string; onChange: (next: string) => void }) {
  const [error, setError] = useState("");
  function pick(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 2 * 1024 * 1024) { setError("Ảnh chữ ký tối đa 2 MB."); return; }
    const reader = new FileReader();
    reader.onload = () => { onChange(String(reader.result || "")); setError(""); };
    reader.readAsDataURL(file);
  }
  return <div className="signature-field" data-vntech="signature-field">
    <div className="signature-preview">{value ? <img src={value} alt="Chữ ký"/> : <span className="muted">Chưa có chữ ký</span>}</div>
    <div><strong>Chữ ký</strong><p>JPG/PNG/WebP · tối đa 2 MB · <b>đúng 1 ảnh</b>; chọn ảnh mới sẽ <b>thay ảnh cũ</b>.</p>
      {error && <small className="red-text">{error}</small>}
      <div className="row-actions">
        <label className="secondary file-inline">{value ? "Thay ảnh chữ ký" : "Chọn ảnh chữ ký"}<input type="file" accept="image/jpeg,image/png,image/webp" onChange={pick}/></label>
        <button type="button" className="secondary" disabled={!value} onClick={() => onChange("")}>Xóa chữ ký</button>
      </div>
    </div>
  </div>;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// MT2-P13-01 (§14) — MODAL THÔNG BÁO HỆ THỐNG khi đăng nhập.
...
//
// ⚠️ PHẠM VI TASK NÀY (P13-01) = **modal + 3 trường hiển thị**. ⛔ Nút «Không nhắc lại hôm nay» =
// `mark_notification_snooze` ⇒ **P13-02**; ⛔ «Đánh dấu đã đọc» = `mark_notification_read` ⇒ **P13-03**.
//    ⇒ Ở task này chỉ có nút **Đóng** (đóng modal ở phiên hiện tại) — ⛔ KHÔNG dựng nút chết, ⛔ KHÔNG
//    tự gắn cờ read/snooze (sẽ là giả lập trạng thái chưa có action).
// 📦 Backend đã LỌC SẴN theo userID + khoảng thời gian còn hiệu lực (`BootstrapDataAdapter.systemNotifications`:
//    `s.user_id=?` + `active=1` + `channel='web'` + `send_at<=now` + `end_at>now` + `read_at IS NULL` +
//    snooze hết hạn + recipient resolver) ⇒ modal chỉ hiện thông báo ĐÚNG cho người đang đăng nhập.
// ─────────────────────────────────────────────────────────────────────────────
function SystemNotificationModal({ notification, close, submit }: { notification: Row; close: () => void; submit: (name: string, payload: Row) => Promise<boolean> }) {
  const [busy, setBusy] = useState(false);
  const [noticeError, setNoticeError] = useState("");
  // MT2-P13-02 (§14) — «Không nhắc lại hôm nay»: `mark_notification_snooze` ghi vào
  // `notification_user_states(config_id, user_id)` ⇒ lưu theo **userID + notificationID** (§14 đúng yêu cầu).
  // ⛔ KHÔNG gửi `userId`: backend luôn lấy `cu.id()` của CHÍNH người đang đăng nhập ⇒ ⛔ không thể snooze
  // thay người khác, ⛔ KHÔNG đánh dấu global cho mọi user.
  // ⛔ KHÔNG gửi `snoozeUntil` để dùng **luật sẵn có** của backend (+24h) — ⛔ không tự đặt luật nghiệp vụ mới.
  async function snoozeToday() {
    setBusy(true);
    setNoticeError("");
    try {
      if (await submit("mark_notification_snooze", { configId: notification.configId })) close();
    } catch (error) {
      setNoticeError(error instanceof Error ? error.message : "Không lưu được trạng thái tạm ẩn.");
    } finally {
      setBusy(false);
    }
  }
  // MT2-P13-03 (§14.1) — «Đánh dấu đã đọc» (đúng 1 thông báo) + «Đánh dấu tất cả đã đọc».
  // ⛔ KHÔNG gửi userId ở CẢ HAI: backend lấy `cu.id()` từ phiên ⇒ trạng thái **theo user** (§14.1) và
  // ⛔ không thể đánh dấu global. `mark_notification_all_read` **không** nhận tham số nào.
  async function markRead() {
    setBusy(true);
    setNoticeError("");
    try {
      if (await submit("mark_notification_read", { configId: notification.configId })) close();
    } catch (error) {
      setNoticeError(error instanceof Error ? error.message : "Không lưu được trạng thái đã đọc.");
    } finally {
      setBusy(false);
    }
  }
  async function markAllRead() {
    ...
    setNoticeError("");
    try {
      if (await submit("mark_notification_all_read", {})) close();
    } catch (error) {
      setNoticeError(error instanceof Error ? error.message : "Không lưu được trạng thái đã đọc.");
    } finally {
      setBusy(false);
  ...
  // ⚠️ Bọc trong `<form>` để nút chính **thực sự** đóng modal: `ModalFooter` dùng nút primary kiểu submit, đặt
  // ngoài form sẽ thành NÚT CHẾT (phát hiện ở P13-02 sau khi P13-01 dùng nó sai cách).
  return <BaseModal title={String(notification.name || "Thông báo hệ thống")} note="Thông báo này được gửi tới đúng tài khoản của bạn." close={close}>
    <form onSubmit={(event) => { event.preventDefault(); close(); }}>
      <div className="modal-body" data-vntech="system-notification-modal">
        <div className="system-notice-meta">
          <span><b>Người tạo:</b> {String(notification.createdBy || "Hệ thống")}</span>
          <span><b>Thời gian phát hành:</b> {date(notification.sendAt)}</span>
        </div>
        <p className="system-notice-content">{String(notification.content || "—")}</p>
        {noticeError && <div className="inline-alert">{noticeError}</div>}
      </div>
      <footer className="modal-footer">
        <button type="button" className="secondary" disabled={busy} onClick={() => void snoozeToday()}>Không nhắc lại hôm nay</button>
        <button type="button" className="secondary" disabled={busy} onClick={() => void markRead()}>Đánh dấu đã đọc</button>
        <button type="button" className="secondary" disabled={busy} onClick={() => void markAllRead()}>Đánh dấu tất cả đã đọc</button>
        <button type="submit" className="primary" disabled={busy}>{busy ? "Đang lưu…" : "Đóng"}</button>
      </footer>
    </form>
  </BaseModal>;
}
