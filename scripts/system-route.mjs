...
const ACTION_CAPABILITY = {
    preview_request_import: "canCreate", create_request: "canCreate", update_returned_request: "canEdit", resubmit_request: "canEdit", delete_request: "canEdit", cancel_request: "canEdit", decide_approval: "canApprove", create_po: "canCreate", save_supplier: "canEdit", set_supplier_status: "canEdit", delete_supplier: "canEdit", save_partner: "canEdit", set_partner_status: "canEdit", delete_partner: "canEdit", save_contract_payment: "canCreate", import_contract_payments: "canCreate", delete_contract_payment: "canEdit", save_production_report: "canCreate", approve_production_report: "canApprove", save_construction_daily_log: "canCreate", approve_construction_daily_log: "canApprove", delete_construction_daily_log: "canEdit", save_capital_recovery: "canCreate", delete_capital_recovery: "canEdit", close_po_line: "canApprove",
...
/**
 * Ghi nhật ký kiểm toán. `AD-14` (PHASE 7 — chỉ đạo người dùng 21/09/2026 «thêm result»): cột `result` là
 * KẾT QUẢ NGHIỆP VỤ của hành động, KHÔNG được rỗng. Từ vựng ĐÓNG (ngắn): `ok` · `denied` · `failed`.
 * Mặc định `ok` vì mọi lời gọi `audit()` hiện nay nằm ở CUỐI nhánh THÀNH CÔNG của action (lỗi thì ném ra
 * trước khi tới đây). Khi cần ghi một lần TỪ CHỐI/LỖI thì truyền `result` tường minh.
 * `metadata` KHÔNG có cột riêng: metadata = CHÍNH `before_json` + `after_json` (người dùng chốt).
 */
async function audit(userId, action, entityType, entityId, before, after, request, result = "ok") {
    await env.DB.prepare(`INSERT INTO audit_logs (id,user_id,action,entity_type,entity_id,before_json,after_json,ip_address,result,occurred_at) VALUES (?,?,?,?,?,?,?,?,?,?)`)
        .bind(id("AUD"), userId, action, entityType, entityId, before ? JSON.stringify(before) : null, after ? JSON.stringify(after) : null, requestIp(request), clean(result) || "ok", now()).run();
}
...
async function bootstrap(user) {
    ...
    let materials = await all(`SELECT m.id,m.code,m.name,m.system,m.category_id AS categoryId,mc.code AS categoryCode,mc.name AS categoryName,m.subcategory_id AS subcategoryId,ms.code AS subcategoryCode,ms.name AS subcategoryName,m.specification,m.brand,m.unit,m.standard_price AS standardPrice,m.min_stock AS minStock,m.requires_cocq AS requiresCocq,m.requires_mar AS requiresMar,m.active AS active FROM materials m LEFT JOIN material_categories mc ON mc.id=m.category_id LEFT JOIN material_subcategories ms ON ms.id=m.subcategory_id WHERE (m.category_id IS NULL OR mc.active=1) AND (m.subcategory_id IS NULL OR ms.active=1) ORDER BY m.active ASC,COALESCE(mc.sort_order,999),COALESCE(ms.sort_order,9999),m.code`);
    ...
    // TASK-125 (21/09/2026) «Đối tác là BẢNG RIÊNG» — ĐỌC THÊM bảng mới `partners` (CHỈ THÊM, không sửa câu `suppliers` ở trên).
    const partners = await all(`SELECT id,code,name,tax_code AS taxCode,address,contact_name AS contactName,contact_phone AS contactPhone,email,partner_type AS partnerType,status,active,created_at AS createdAt,updated_at AS updatedAt FROM partners WHERE active=1 ORDER BY code`);
    const adminPartners = isAdmin(user) ? await all(`SELECT id,code,name,tax_code AS taxCode,address,contact_name AS contactName,contact_phone AS contactPhone,email,partner_type AS partnerType,status,active,created_at AS createdAt,updated_at AS updatedAt FROM partners ORDER BY CASE WHEN active=1 THEN 0 ELSE 1 END,code`) : partners;
    ...
    const audits = isAdmin(user) ? await all(`SELECT al.id,al.action,al.entity_type AS entityType,al.entity_id AS entityId,al.occurred_at AS occurredAt,al.result,al.before_json AS beforeJson,al.after_json AS afterJson,u.full_name AS userName FROM audit_logs al LEFT JOIN users u ON u.id=al.user_id ORDER BY al.occurred_at DESC LIMIT 100`) : [];
...
async function handleAction(action, payload, user, request) {
    ...
    // ─────────────────────────────────────────────────────────────────────────────
    // TASK-125 (21/09/2026) — «ĐỐI TÁC LÀ BẢNG RIÊNG» (quyết định nguyên văn của người dùng).
    // 3 action MỚI, CHỈ THÊM — không sửa `save_supplier`/`set_supplier_status`/`delete_supplier` ở trên.
    // Bảng: `partners` (migration `drizzle/0165_task125_partners_table_identity.sql` + Flyway `V23__partners_table.sql`).
    // Quyền: dùng chung khoá CŨ `supplier_catalog` + `dept_plan_suppliers` ⇒ 0 khoá `module_catalog` mới.
    // ─────────────────────────────────────────────────────────────────────────────
    if (action === "save_partner") {
        const partnerId = clean(payload.partnerId), code = clean(payload.code).toUpperCase(), name = clean(payload.name);
        if (!code || !name) throw new Error("Đối tác phải có Mã đối tác và Tên đối tác.");
        const existing = await first(`SELECT id FROM partners WHERE upper(code)=upper(?) AND id<>?`, code, partnerId || "__NEW__");
        if (existing) throw new Error("Mã đối tác đã tồn tại.");
        const taxCode = clean(payload.taxCode) || null, address = clean(payload.address) || null, contactName = clean(payload.contactName) || null;
        const contactPhone = clean(payload.contactPhone) || null, email = clean(payload.email) || null;
        const partnerType = clean(payload.partnerType) || "supplier", active = payload.active === false || clean(payload.active) === "0" ? 0 : 1;
        const status = clean(payload.status) || (active ? "active" : "inactive");
        if (partnerId) { const old = await first(`SELECT * FROM partners WHERE id=?`, partnerId); if (!old) throw new Error("Không tìm thấy đối tác."); await env.DB.prepare(`UPDATE partners SET code=?,name=?,tax_code=?,address=?,contact_name=?,contact_phone=?,email=?,partner_type=?,status=?,active=?,updated_at=? WHERE id=?`).bind(code,name,taxCode,address,contactName,contactPhone,email,partnerType,status,active,stamp,partnerId).run(); await audit(user.id,"UPDATE","partner",partnerId,old,{code,name,active},request); return { message: `Đã cập nhật đối tác ${code}.` }; }
        const newId = id("PTR"); await env.DB.prepare(`INSERT INTO partners (id,code,name,tax_code,address,contact_name,contact_phone,email,partner_type,status,active,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(newId,code,name,taxCode,address,contactName,contactPhone,email,partnerType,status,active,stamp,stamp).run(); await audit(user.id,"CREATE","partner",newId,null,{code,name},request); return { message: `Đã thêm đối tác ${code}.` };
    }
    if (action === "set_partner_status") { const partnerId=clean(payload.partnerId), active=payload.active===true||clean(payload.active)==="1"?1:0; const old=await first(`SELECT * FROM partners WHERE id=?`,partnerId);if(!old)throw new Error("Không tìm thấy đối tác.");await env.DB.prepare(`UPDATE partners SET active=?,status=?,updated_at=? WHERE id=?`).bind(active,active?"active":"inactive",stamp,partnerId).run();await audit(user.id,active?"ACTIVATE":"HIDE","partner",partnerId,old,{active},request);return {message:active?"Đã kích hoạt đối tác.":"Đã chuyển đối tác sang Ngừng sử dụng."};
    }
    if (action === "delete_partner") {
        if(!isAdmin(user) && !isDepartmentApprover(user,"KH")) throw new Error("Chỉ Quản trị viên hoặc Trưởng phòng Kế hoạch được phân quyền mới được xóa/ngừng sử dụng Đối tác.");
        const partnerId=clean(payload.partnerId); const old=await first(`SELECT * FROM partners WHERE id=?`,partnerId); if(!old) throw new Error("Không tìm thấy đối tác.");
        await env.DB.prepare(`DELETE FROM partners WHERE id=?`).bind(partnerId).run(); await audit(user.id,"DELETE","partner",partnerId,old,null,request); return {message:"Đã xóa Đối tác."};
    }
    ...
    if (action === "save_legal_document") {
        // MT2 §10.4 (đề án ②A) — thêm `correspondenceId` (khoá văn bản pháp lý ↔ công văn) để giữ PARITY với Java.
        const docId=clean(payload.docId),docNo=clean(payload.docNo),docType=clean(payload.docType),title=clean(payload.title),issueDate=clean(payload.issueDate)||null,issuer=clean(payload.issuer)||null,effectiveDate=clean(payload.effectiveDate)||null,expiryDate=clean(payload.expiryDate)||null,scope=clean(payload.scope)||null,attachmentId=clean(payload.attachmentId)||null,correspondenceId=clean(payload.correspondenceId)||null;
        if(!docNo||!docType||!title)throw new Error("Văn bản pháp lý cần số, loại và tiêu đề.");
        if(docId){const old=await first(`SELECT * FROM legal_documents WHERE id=?`,docId);if(!old)throw new Error("Không tìm thấy văn bản.");await env.DB.prepare(`UPDATE legal_documents SET doc_no=?,doc_type=?,title=?,issue_date=?,issuer=?,effective_date=?,expiry_date=?,scope=?,attachment_id=?,correspondence_id=?,updated_at=? WHERE id=?`).bind(docNo,docType,title,issueDate,issuer,effectiveDate,expiryDate,scope,attachmentId,correspondenceId,stamp,docId).run();await audit(user.id,"UPDATE","legal_document",docId,old,{docNo,correspondenceId},request);return {message:"Đã cập nhật văn bản pháp lý."};}
        const dup=await first(`SELECT id FROM legal_documents WHERE doc_no=?`,docNo);if(dup)throw new Error("Số văn bản đã tồn tại.");const newId=id("LGD");await env.DB.prepare(`INSERT INTO legal_documents(id,doc_no,doc_type,title,issue_date,issuer,effective_date,expiry_date,scope,attachment_id,correspondence_id,status,created_by,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(newId,docNo,docType,title,issueDate,issuer,effectiveDate,expiryDate,scope,attachmentId,correspondenceId,"active",user.id,stamp,stamp).run();await audit(user.id,"CREATE","legal_document",newId,null,{docNo,correspondenceId},request);return {message:"Đã lưu văn bản pháp lý."};
    }
    ...
    if (action === "delete_project") {
        ...
        const auditId=id("AUD");await env.DB.prepare(`INSERT INTO audit_logs (id,user_id,action,entity_type,entity_id,before_json,after_json,ip_address,result,occurred_at) VALUES (?,?,?,?,?,?,?,?,?,?)`).bind(auditId,user.id,"PURGE_AFTER_OFFLINE_ARCHIVE","project",projectId,JSON.stringify(project),JSON.stringify({archiveId:archive.id,archiveFile:archive.fileName,archiveSha256:archive.sha256,status:"purged",crossProjectTracePreserved:true,objectCleanupFailed}),requestIp(request),"ok",stamp).run();
