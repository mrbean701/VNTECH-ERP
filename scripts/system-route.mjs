...
async function handleAction(action, payload, user, request) {
    ...
    if (action === "receive_goods") {
        ...
        return { message: posting === "posted" ? `Đã ghi nhận ${receiptNo}; đơn tự chuyển sang chờ BCH tải/kiểm tra ảnh và xác nhận.` : `${receiptNo} đã lưu nhưng bị chặn ghi sổ do QC hoặc hồ sơ chưa đạt; BCH vẫn phải xác nhận tình trạng thực tế.` };
    }
