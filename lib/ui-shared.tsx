...
type AppData = {
  ...
  // MT2-P13-01 (§14) — thông báo HỆ THỐNG cần hiện khi đăng nhập. ⛔ BẮT BUỘC (không `?`): thiếu khai báo ở
  // đây thì `tsc` không bắt được lỗi «đường ĐỌC thiếu khoá» ⇒ sẽ hiện mảng rỗng âm thầm (bài học TASK-069).
...
function AttachmentPanel({ entityType, entityId, canManage=true }: { entityType: string; entityId: string; canManage?: boolean }) {
  ...
  // MT2 §27 — “Bảng ảnh → click → Full image viewer”. Trước đây mở TAB MỚI; nay mở LIGHTBOX trong app.
  ...
  // MT2 §6.12/§26 — cho phép chọn NHIỀU ảnh/hồ sơ cùng lúc. Backend `/api/files` nhận 1 tệp/lần
  // ⇒ gửi lần lượt nhiều request, KHÔNG đổi API. Báo rõ số tệp thành công/thất bại (⛔ không báo sai).
  async function upload(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const formElement = event.currentTarget; const picked = Array.from((formElement.elements.namedItem("file") as HTMLInputElement | null)?.files || []); if (!picked.length) { setStatus("Chưa chọn tệp nào."); return; } setStatus(`Đang tải ${picked.length} tệp…`); const errors: string[] = []; let ok = 0; for (const one of picked) { const form = new FormData(); form.set("file", one); form.set("entityType", entityType); form.set("entityId", entityId); try { const response = await fetch("/api/files", { method: "POST", body: form }); if (response.ok) { ok += 1; } else { const result = await response.json().catch(() => ({ error: "Không tải được tệp." })); errors.push(`${one.name}: ${String(result.error || "lỗi")}`); } } catch { errors.push(`${one.name}: lỗi mạng`); } } formElement.reset(); await loadFiles(); setStatus(errors.length ? `Đã tải ${ok}/${picked.length} tệp. Lỗi: ${errors.join("; ")}` : `Đã lưu ${ok} tệp.`); }
  ...
  return <div className="attachment-panel">{canManage?<form className="file-upload" onSubmit={upload}><label className="attachment-pick"><span>Chọn ảnh / hồ sơ</span><input name="file" type="file" required multiple accept=".pdf,.jpg,.jpeg,.png,.webp,.xlsx,.doc,.docx" /></label><button className="secondary">↑ TẢI LÊN</button>{status && <span>{status}</span>}</form>:<div className="attachment-readonly-note">Chỉ Chỉ huy trưởng hoặc Nhân viên Phòng Dự án được bổ sung/xóa hồ sơ vật tư đặc thù. Bạn có thể xem và tải xuống.</div>}{imageFiles.length > 0 && <div className="attachment-photos">{imageFiles.map((file) => <button type="button" className="attachment-photo" key={file.id} onClick={()=>setPreview(file)} title={file.fileName}><img src={`/api/files?id=${encodeURIComponent(file.id)}`} alt={file.fileName} loading="lazy"/><span>{file.fileName}</span></button>)}</div>}<div className="attachment-list">{files.map((file) => <div className="attachment-row" key={file.id}><a href={`/api/files?id=${encodeURIComponent(file.id)}`} target="_blank" rel="noreferrer" onClick={(event)=>{if(String(file.mimeType||"").startsWith("image/")){event.preventDefault();setPreview(file);}}}><span>{String(file.mimeType).startsWith("image/") ? <img className="attachment-thumb" src={`/api/files?id=${encodeURIComponent(file.id)}`} alt={file.fileName} loading="lazy"/> : "TỆP"}</span><div><strong>{file.fileName}</strong><small>{file.uploadedByName} · {date(file.createdAt)}</small></div><b>↓</b></a>{canManage&&<button type="button" className="attachment-delete" onClick={()=>void removeFile(file)}>Xóa</button>}</div>)}{!files.length && <small>Chưa có ảnh hoặc hồ sơ vật tư đặc thù được tải lên.</small>}</div>{preview&&<div className="attachment-lightbox" data-vntech="attachment-lightbox" role="dialog" aria-modal="true" aria-label="Xem ảnh" onClick={()=>setPreview(null)}><img src={`/api/files?id=${encodeURIComponent(String(preview.id))}`} alt={String(preview.fileName||"")} onClick={(event)=>event.stopPropagation()}/><div className="attachment-lightbox-bar" onClick={(event)=>event.stopPropagation()}><b>{String(preview.fileName||"")}</b><a href={`/api/files?id=${encodeURIComponent(String(preview.id))}`} target="_blank" rel="noreferrer">Mở tab mới / tải xuống</a><button type="button" onClick={()=>setPreview(null)}>✕ Đóng (ESC)</button></div></div>}</div>;
}
